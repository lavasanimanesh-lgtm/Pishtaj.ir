...
**هات‌فیکس 9.0.1 دامنهٔ نهایی:** (۱) snapshot از لایهٔ داده/آینه یا حذف وقتی آینه فعال + سقف بایت؛ (۲) تشخیص و اعلام کلید dirty گیرکرده (در جعبهٔ وضعیت دستگاه — بدون کنسول) + مسیر رهایش؛ (۳) آیتم‌های سبزِ‌ناتمام قبلی. **درمان بدون دیپلوی (تأییدشده):** اگر صف آفلاین صفر است، پاک‌کردن دادهٔ سایت (Safari/Chrome iOS) + ورود دوباره ⇒ bootstrap خودکار همگرایی خالی ⇒ markSynced ⇒ آینه فعال ⇒ LS سبک می‌ماند. **✅ ساخته شد — v34.9.1 (2026-08-31):** SNAP-PUMP (سقف ۲۵۶KB/۳ نسخه/انقضای ۲۴س + مسیر IDB وقتی آینه فعال) + جعبهٔ سه‌حالته با «تکمیل انتقال» + بنر برای حالت ناتمام + ptfSyncDirtyInfo/رهایش امن audit/notifs/avatars + بنر قرمز با علت دقیق (Quota/Security) + dirty timestamp. tester553 (۳۱ سنجه)؛ گیت ۱۶۹/۰؛ arch PASS. حذف موتور legacy + A10 + T3-5 → **v34.9.2** (شرط ≥۷ روز سبز R4). پیشنهاد قبلی (جراحی‌شده): دکمهٔ «تکمیل انتقال» برای enabled&&!synced + بنر برای همان حالت + پیام خطای ذخیره با علت دقیق. مترهای پنجرهٔ R4 (win7/بایپس) با این تغییر دست نمی‌خورند.
