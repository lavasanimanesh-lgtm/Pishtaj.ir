...
- **RCA روز دیپلوی ۲ (→ v34.9.1):** همگرایی یک‌باره روی دستگاه مدیرِ سبزِ‌ناتمام (فعال‌شده با دکمهٔ قدیمی، بدون ptf_b_synced): ۴۸/۴۹ کلید ACK، فقط `ptf_crm_avatars` در سبد rejected سرور (گارد data_push؛ کلید مشترک/آرایشی، سرور نسخهٔ مرجع را دارد). درمان عملی: حذف امن نسخهٔ محلی legacy (با بک‌آپ sessionStorage) + تکمیل همگرایی. **اصلاح 9.1:** (الف) جعبهٔ سبزِ ناتمام (enabled && !synced) دکمهٔ «تکمیل انتقال» نشان دهد؛ (ب) همگرایی کلید avatars را از payload مستثنا یا اول سبک‌سازی کند (قرارداد S3/نقشهٔ {k,t,ts} در 8.44)؛ (ج) پاک‌سازی snapهای pre_pull قدیمی؛ (د) عنوان ردیف + بازشدن خودکار توسط بنر.
