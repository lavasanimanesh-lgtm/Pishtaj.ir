...
| P1 | `US-435` | اتصال UI و predicate فعال مشتری در `v31.6.14` انجام شد؛ full regression هنوز fail است | targeted UAT و رفع failureهای معتبر پیش از ZIP Production |
...
| اولویت | شناسه | وضعیت جاری | اقدام بعدی |
|---|---|---|---|
| P0 | `BUG-PROC-LINK-287 / FIN-WF-019` | resolver و لایهٔ انتخاب خرید واقعی با provenance پایدار در `v31.6.14` تکمیل شد؛ دادهٔ legacy بدون provenance عمداً انتخاب نمی‌شود | UAT و بررسی موارد legacy مبهم، بدون migration خودکار |
...
| اولویت | شناسه | وضعیت جاری | اقدام بعدی |
|---|---|---|---|
| P1 | `US-437` | در `v31.6.14` rollback شده؛ فقط مستندات/کیس‌استادی | parked تا approval مستقل و طراحی workflow |
...
Hardening baseline فعلی تکمیل شده و baseline v31.7 اکنون در وضعیت `Prepared / Blocked for Active Evidence` شروع شده است. اجرای PoC فعال همچنان به تصمیم policy چک شخصی و staging مستقل وابسته است؛ US-436/437 و migration داده وارد آن نشده‌اند.
