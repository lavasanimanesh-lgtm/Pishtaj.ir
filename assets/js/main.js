...
const slides=[...document.querySelectorAll('.slide')],dotsWrap=document.getElementById('sliderDots');if(dotsWrap&&slides.length){let current=0;slides.forEach((_,i)=>{const b=document.createElement('button');b.type='button';b.setAttribute('aria-label','نمایش اسلاید '+(i+1));b.addEventListener('click',()=>goSlide(i));dotsWrap.appendChild(b)});const dots=[...dotsWrap.children];function goSlide(i){slides[current]?.classList.remove('active');dots[current]?.classList.remove('active');current=i;slides[current].classList.add('active');dots[current].classList.add('active')}goSlide(0);setInterval(()=>goSlide((current+1)%slides.length),5400);}
const reveals=document.querySelectorAll('.reveal');const io=new IntersectionObserver(es=>es.forEach(e=>{if(e.isIntersecting)e.target.classList.add('visible')}),{threshold:.12});reveals.forEach(el=>io.observe(el));
