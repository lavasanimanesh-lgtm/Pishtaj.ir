...
SLUGS = ['radar-level-transmitter']


class RadarExistingSEOTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_no_editorial_boilerplate_or_stock_promises(self):
        for slug, text in self.pages.items():
            ...
            self.assertEqual(text.count('data-radar-seo="2026-09-20"'), 1)
    ...
    def test_specific_technical_corrections_and_sources(self):
        text = self.pages['radar-level-transmitter']
        for phrase in ['زاویه پرتو به نوع و ابعاد آنتن نیز وابسته است', 'گواهی SIL یک تجهیز، تأیید کل سامانه جلوگیری از سرریز نیست', 'False Echo Mapping جایگزین اصلاح نصب نامناسب نیست', 'اندازه‌گیری هم‌زمان سطح کل و Interface با حالت پروب کاملاً غوطه‌ور یکسان نیست']:
            self.assertIn(phrase, text)
        self.assertIn('dl-rmt-5300-level-transmitter-guided-wave-radar-78542.pdf', text)
        self.assertIn('https://www.vega.com/', text)
        self.assertIn('data-deep-upgrade="radar-level-deep-2026"', text)
        for target in ['displacer-level-transmitter.html', 'differential-pressure-transmitter.html']:
    ...
    def test_container_balance_and_table_semantics(self):
        for slug, text in self.pages.items():
            ...
            tables = re.findall(r'<div class="ptf-radar-table">(.*?)</div>', text, re.S)
