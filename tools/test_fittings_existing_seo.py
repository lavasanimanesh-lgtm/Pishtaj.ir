"""Offline checks for strengthening existing flange and fitting pages only."""
import json
import re
import unittest
...
SLUGS = ['welding-flanges', 'butt-weld-fittings', 'forged-fittings']


class FittingsExistingSEOTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_no_editorial_boilerplate_or_stock_promises(self):
        for slug, text in self.pages.items():
            ...
            self.assertEqual(text.count('data-fitting-seo="2026-09-19"'), 1)
            self.assertIn('قیمت قطعی و موجودی تضمین‌شده', body)
            self.assertIn('مدت اعتبار قیمت', body)
            self.assertIn('مالیات', body)

    def test_specific_technical_corrections_and_sources(self):
        forged = self.pages['forged-fittings']
        sw = re.search(r'<th scope="row">Socket Weld طبق B16.11</th><td>(.*?)</td>', forged)[1]
        threaded = re.search(r'<th scope="row">Threaded طبق B16.11</th><td>(.*?)</td>', forged)[1]
        for number in ['3000', '6000', '9000']:
            self.assertIn(number, sw)
        for number in ['2000', '3000', '6000']:
            self.assertIn(number, threaded)
        self.assertNotIn('9000', threaded)
        self.assertNotIn('2000', sw)
        self.assertNotIn('Class 3000/6000/9000', forged)
        self.assertIn('عدد Class را نباید فشار مجاز ثابت', forged)
        self.assertIn('دامنه همه کلاس‌ها یکسان نیست', self.pages['welding-flanges'])
        self.assertIn('BW نوع اتصال جوش لب‌به‌لب', self.pages['butt-weld-fittings'])
        for text in self.pages.values():
            self.assertIn('https://www.asme.org/codes-standards/find-codes-standards/', text)
            self.assertIn('ویرایش قراردادی', text)
    ...
    def test_container_balance_and_table_semantics(self):
        for slug, text in self.pages.items():
            ...
            tables = re.findall(r'<div class="ptf-fitting-table">(.*?)</div>', text, re.S)
