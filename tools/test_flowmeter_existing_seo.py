...
class FlowmeterExistingSEOTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_no_editorial_boilerplate_or_stock_promises(self):
        for slug, text in self.pages.items():
            ...
            self.assertEqual(text.count('data-flow-seo="2026-09-20"'), 1)
    ...
    def test_specific_technical_corrections_and_sources(self):
        text = self.pages['flowmeter']
        for phrase in ['دبی جرمی', 'شرایط مرجع', 'Rangeability', 'تکرارپذیری نیز با صحت اندازه‌گیری یکی نیست', 'ترانسمیتر DP تنها یک دبی‌سنج کامل نیست']:
            self.assertIn(phrase, text)
        for link in ['magnetic-flowmeter.html', 'coriolis-flowmeter.html', 'vortex-flowmeter.html', 'differential-pressure-transmitter.html']:
            self.assertIn('href="' + link + '"', text)
        self.assertIn('https://www.endress.com/en/field-instruments-overview/flow-measurement-product-overview/electromagnetic-flowmeters', text)
    ...
    def test_container_balance_and_table_semantics(self):
        for slug, text in self.pages.items():
            ...
            tables = re.findall(r'<div class="ptf-flow-table">(.*?)</div>', text, re.S)
