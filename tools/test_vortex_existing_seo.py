...
SLUGS = ['vortex-flowmeter']


class VortexExistingSEOTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_no_editorial_boilerplate_or_stock_promises(self):
        for slug, text in self.pages.items():
            ...
            self.assertEqual(text.count('data-vortex-seo="2026-09-20"'), 1)
    ...
    def test_specific_technical_corrections_and_sources(self):
        text = self.pages['vortex-flowmeter']
        for phrase in ['فشار و دما به‌تنهایی کیفیت بخار مرطوب را تعیین نمی‌کنند', 'فشار مطلق', 'ضریب پالس خروجیِ قابل تنظیم الزاماً همان ضریب کالیبراسیون سنسور نیست', 'عدد ثابت طول مستقیم برای همه نصب‌ها مناسب نیست']:
            self.assertIn(phrase, text)
        self.assertIn('https://www.endress.com/en/field-instruments-overview/flow-measurement-product-overview/vortex-flowmeters', text)
        self.assertIn('data-deep-upgrade="vortex-flowmeter-deep-2026"', text)
        for target in ['pressure-transmitter.html', 'temperature-transmitter.html']:
    ...
    def test_container_balance_and_table_semantics(self):
        for slug, text in self.pages.items():
            ...
            tables = re.findall(r'<div class="ptf-vortex-table">(.*?)</div>', text, re.S)
