"""Offline checks for strengthening existing pressure measurement pages only."""
import json
import re
import unittest
...
SLUGS = ['pressure-transmitter', 'differential-pressure-transmitter', 'diaphragm-seal']


class PressureExistingSEOTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_no_editorial_boilerplate_or_stock_promises(self):
        for slug, text in self.pages.items():
            ...
            self.assertEqual(text.count('data-pressure-seo="2026-09-19"'), 1)
    ...
    def test_specific_technical_corrections_and_sources(self):
        pressure = self.pages['pressure-transmitter']
        dp = self.pages['differential-pressure-transmitter']
        seal = self.pages['diaphragm-seal']
        self.assertIn('حد تحمل فشار سخت‌افزار را تغییر نمی‌دهد', pressure)
        self.assertIn('فشار استاتیک', dp)
        self.assertIn('Square Root Extraction', dp)
        self.assertIn('فقط یک‌بار', dp)
        self.assertIn('سیستم بسته پرشده‌اند', seal)
        self.assertIn('ds_in0006_en_co.pdf', seal)
        self.assertNotIn('data-final-stability=', seal)
        self.assertNotIn('data-final-lowdepth', seal)
        for page in [pressure, dp]:
            self.assertIn('https://www.emerson.com/', page)
            self.assertIn('دقت مرجع', page)
    ...
    def test_container_balance_and_table_semantics(self):
        for slug, text in self.pages.items():
            ...
            tables = re.findall(r'<div class="ptf-pressure-table">(.*?)</div>', text, re.S)
