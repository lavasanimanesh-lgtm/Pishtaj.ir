...
SLUGS = ['magnetic-flowmeter']


class MagmeterExistingSEOTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_no_editorial_boilerplate_or_stock_promises(self):
        for slug, text in self.pages.items():
            ...
            self.assertEqual(text.count('data-magmeter-seo="2026-09-20"'), 1)
    ...
    def test_specific_technical_corrections_and_sources(self):
        text = self.pages['magnetic-flowmeter']
        for phrase in ['وجود Virtual Reference به معنی حذف ارت حفاظتی نیست', 'Remote بودن به‌تنهایی مجوز غوطه‌وری', 'Empty Pipe Detection', 'لاینر و الکترود دو انتخاب مستقل', 'قانون فارادی اصل فیزیکی عملکرد است']:
            self.assertIn(phrase, text)
        for url in ['https://www.krohne.com/en-us/technologies/virtual-reference', 'https://www.endress.com/en/field-instruments-overview/flow-measurement-product-overview/electromagnetic-flowmeters']:
            self.assertIn(url, text)
        self.assertIn('data-deep-upgrade="magnetic-flowmeter-deep-2026"', text)
        self.assertIn('href="coriolis-flowmeter.html"', text)
        self.assertNotIn('data-final-stability=', text)
        self.assertNotIn('data-final-lowdepth', text)
        self.assertNotIn('Insertion Magmeter برای خطوط بزرگ با دقت کمتر', text)
    ...
    def test_container_balance_and_table_semantics(self):
        for slug, text in self.pages.items():
            ...
            tables = re.findall(r'<div class="ptf-magmeter-table">(.*?)</div>', text, re.S)
