"""Offline checks for strengthening existing pressure gauge pages only."""
import json
import re
import unittest
...
SLUGS = ['pressure-gauge']


class GaugeExistingSEOTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_no_editorial_boilerplate_or_stock_promises(self):
        for slug, text in self.pages.items():
            ...
            self.assertEqual(text.count('data-gauge-seo="2026-09-20"'), 1)
    ...
    def test_specific_technical_corrections_and_sources(self):
        text = self.pages['pressure-gauge']
        for phrase in ['گیج روغنی با Safety Pattern یکسان نیست', '±۰٫۱ بار', 'درپوش تخلیه به‌تنهایی معادل Solid Front نیست', 'صفرخواندن عقربه به‌تنهایی نبود فشار محبوس را ثابت نمی‌کند']:
            self.assertIn(phrase, text)
        self.assertIn('https://blog.wika.com/en/knowhow/stainless-steel-pressure-gauge-in-safety-version-selection-help/', text)
    ...
    def test_container_balance_and_table_semantics(self):
        for slug, text in self.pages.items():
            ...
            tables = re.findall(r'<div class="ptf-gauge-table">(.*?)</div>', text, re.S)
