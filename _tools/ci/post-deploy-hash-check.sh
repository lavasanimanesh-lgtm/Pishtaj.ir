...
check_one() {
  local rel="$1" base="$2" url expected actual tmp_live attempt rc
  local local_path="$ROOT_DIR/$rel"
...
self_test() {
  ...
  mkdir -p "$tmp/live/crm" "$tmp/repo/crm" "$tmp/repo/_tools/ci"
...
JS
  ...
  run() { # $1=انتظار-exit $2=توضیح  (فلگ‌ها از VARهای از پیش ست‌شده)
    local got
    ( cd "$tmp/repo" && bash "$tmp/repo/_tools/ci/post-deploy-hash-check.sh" \
        --base "http://127.0.0.1:${port}" --files "${CASE_FILES:-crm/sw.js}" --retries 1 --sleep 0 ${EXTRA:-} ) >"$tmp/log" 2>&1
    got=$?
    if [ "$got" = "$1" ]; then echo "PASS $2 [exit=$got]"; else echo "FAIL $2 — expected exit=$1 got=$got"; sed 's/^/      | /' "$tmp/log" | head -6; rc=1; fi
  }
  ...
  CASE_FILES="crm/sw.js" EXTRA="--base http://127.0.0.1:1" run 1 "پایهٔ مرده ⇒ شکست"
...
echo "── post-deploy-hash-check · base=${BASE} · policy=${POLICY} · retries=${RETRIES} ──"
