...
# Replace offRenderItems cleanly
start_idx = of.find("function offRenderItems() {")
end_idx = of.find("function offSetRefPrice(", start_idx)
if start_idx != -1 and end_idx != -1:
    clean_off_render = """function offRenderItems() {
  var el = document.getElementById('offItemsWrap');
  if (!el || !_offState) return;
  var isCO = _offState.kind === 'CO' || _offState.kind === 'TC';
  var ec = _offState.extraCols || [];
  var ecHead = ec.map(function(c){ return '<th>' + escP(c) + '</th>'; }).join('');
  var head = isCO
    ? '<tr><th>#</th><th>نام کالا</th><th>شرح تکمیلی</th><th>مدل</th><th>تعداد</th><th>واحد</th><th>برند</th>' + ecHead + '<th style="background:#f5f3ff;color:#5b21b6" title="نرخ مرجع خرید از بانک کالا">نرخ مرجع خرید</th><th style="background:#fef3c7;color:#92400e" title="درصد حاشیه سود">(٪) سود</th><th>قیمت واحد (IRR)</th><th>جمع</th><th>✕</th></tr>'
    : '<tr><th>#</th><th>نام کالا</th><th>شرح تکمیلی</th><th>مدل</th><th>تعداد</th><th>واحد</th><th>برند</th>' + ecHead + '<th>✕</th></tr>';

  var rows = '';
  _offState.items.forEach(function(it, i) {
    function inp(k, w, tp) {
      return '<input type="' + (tp||'text') + '" value="' + escP(it[k]||'') + '" oninput="offUpd(' + i + ',\'' + k + '\',this.value)" style="width:' + w + ';direction:' + (tp==='number'?'ltr':'') + '">';
    }
    var ecCells = ec.map(function(c){
      return '<td><input type="text" value="' + escP((it.extra||{})[c]||'') + '" oninput="offUpdExtra(' + i + ',\'' + escP(c) + '\',this.value)" style="width:80px"></td>';
    }).join('');
    ...
    if (isCO) {
      ...
      rows += '<tr><td>' + (i + 1) + '</td><td>' + inp('name', '100%') + '</td><td>' + inp('desc', '100%') + '</td>' +
        ...
        '<td style="background:#fcfaff"><input type="number" value="' + (_refP || '') + '" oninput="offUpdRefPrice(' + i + ',this.value)" placeholder="نرخ مرجع" style="width:95px;direction:ltr;background:#f5f3ff;border:1px solid #ddd6fe;font-weight:bold;color:#5b21b6;padding:5px;border-radius:6px"></td>' +
        ...
        '<td>' + inp('price', '92px', 'number') + bestBuyHtml + '</td><td id="offRT' + i + '" style="white-space:nowrap;font-size:12px">' + ((+it.qty||0)*(+it.price||0)).toLocaleString('en-US') + '</td>' +
  ...
  el.innerHTML = globalMarginBar + '<table style="width:100%;border-collapse:collapse;font-size:12.5px"><thead style="background:#f1f5f9">' + head + '</thead><tbody>' + rows + totalRow + '</tbody></table>' +
    ...
    of = of[:start_idx] + clean_off_render + of[end_idx:]
    with open(offers_path, "w", encoding="utf-8") as f: f.write(of)
    print("Updated offRenderItems with complete Reference Price & Margin columns + Global Margin bar!")
