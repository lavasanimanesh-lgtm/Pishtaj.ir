#!/usr/bin/env node
'use strict';
/* v34.34.0 — LETTER-AWARD-OPEX-001
...
console.log('\n── پین نسخه و rollover ──');
T('نسخهٔ رسمی دقیقاً v34.34.0 است', version.crm_version === 'v34.34.0', version.crm_version);
T('index، service worker، manifest و API هم‌نسخه‌اند',
  read('crm/index.html').indexOf("window.PTF_CRM_RELEASE = 'v34.34.0'") > -1 &&
  read('crm/sw.js').indexOf("RELEASE = 'v34.34.0'") > -1 &&
  JSON.parse(read('crm/manifest.json')).version === '34.34.0' &&
  api.indexOf("SD_SERVICE_VERSION = '34.34.0'") > -1);
...
console.log('\n— tester502 (v34.34.0: مکاتبات، اسناد برنده و تسویه OPEX) —');
