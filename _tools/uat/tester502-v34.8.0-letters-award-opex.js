...
console.log('\n── پین نسخه و rollover ──');
T('نسخهٔ رسمی دقیقاً v34.9.2 است', version.crm_version === 'v34.9.2', version.crm_version);
T('index، service worker، manifest و API هم‌نسخه‌اند',
  read('crm/index.html').indexOf("window.PTF_CRM_RELEASE = 'v34.9.2'") > -1 &&
  read('crm/sw.js').indexOf("RELEASE = 'v34.9.2'") > -1 &&
  JSON.parse(read('crm/manifest.json')).version === '34.9.2' &&
  api.indexOf("SD_SERVICE_VERSION = '34.9.2'") > -1);
...
console.log('\n— tester502 (v34.9.2: مکاتبات، اسناد برنده و تسویه OPEX) —');
