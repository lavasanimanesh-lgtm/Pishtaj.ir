#!/usr/bin/env node
'use strict';
/* v34.36.4 — LETTER-AWARD-OPEX-001
...
console.log('\n── پین نسخه و rollover ──');
T('نسخهٔ رسمی دقیقاً v34.36.4 است', version.crm_version === 'v34.36.4', version.crm_version);
T('index، service worker، manifest و API هم‌نسخه‌اند',
  read('crm/index.html').indexOf("window.PTF_CRM_RELEASE = 'v34.36.4'") > -1 &&
  read('crm/sw.js').indexOf("RELEASE = 'v34.36.4'") > -1 &&
  JSON.parse(read('crm/manifest.json')).version === '34.36.4' &&
  api.indexOf("SD_SERVICE_VERSION = '34.36.4'") > -1);
...
console.log('\n— tester502 (v34.36.4: مکاتبات، اسناد برنده و تسویه OPEX) —');
