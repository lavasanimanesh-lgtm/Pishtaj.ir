...
console.log('\n── پین نسخه و rollover ──');
T('نسخهٔ رسمی دقیقاً v34.8.8 است', version.crm_version === 'v34.8.8', version.crm_version);
T('index، service worker، manifest و API هم‌نسخه‌اند',
  read('crm/index.html').indexOf("window.PTF_CRM_RELEASE = 'v34.8.8'") > -1 &&
  read('crm/sw.js').indexOf("RELEASE = 'v34.8.8'") > -1 &&
  JSON.parse(read('crm/manifest.json')).version === '34.8.8' &&
  api.indexOf("SD_SERVICE_VERSION = '34.8.8'") > -1);
...
console.log('\n— tester502 (v34.8.8: مکاتبات، اسناد برنده و تسویه OPEX) —');
