...
'use strict';
/* =============================================================================
   tester664 — v34.39.4 (HOME-DOCK-SUN)
   دستور کارفرما:
...
T('۳.۶ استایل ردیف منو (id برنده) + دسکتاپ قرصی + killِ reduce',
  ...
  lay.indexOf('@media(min-width:791px){\n  #mainNav .theme-toggle{height:34px') > -1 &&
...
T('۴.۲ حلقهٔ دائم: ۸ ثانیه (از ۳.۴) — دیربه‌دیرتر و یواش‌تر',
  /animation:ptfCurS 8s linear infinite;opacity:\.75/.test(home) && home.indexOf('animation:ptfCur 3.4s') === -1);
T('۴.۳ حلقهٔ هاور کارت‌ها: ۵ ثانیه (از ۲.۲)',
  /animation:ptfCurS 5s linear infinite/.test(home) && home.indexOf('animation:ptfCur 2.2s') === -1);
...
T('۵.۸ VERSION.json = v34.39.4', JSON.parse(read('VERSION.json')).crm_version === 'v34.39.4');

DONE('tester664-v34.39.4-dock-sun');
