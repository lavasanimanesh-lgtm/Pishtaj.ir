...
(function () {
  ...
  /* v34.39.40 (CONTACT-STALE-HOOK — RCA 2026-09-17): قرارداد ۲.۹ قدیمی («بدون cd،
  ...
  T('۲.۹ بدون cd، هوک هیچ بازنویسی‌ای نمی‌کند (payload اصلی در-flight است — v34.39.40)',
  ...
  T('۲.۱۰ بدون cd و بدون lastSaved هیچ upsert/items[0] نیست',
    origCalls.length === 1 && upserts.length === 0, JSON.stringify(upserts));
  upserts.length = 0; origCalls.length = 0;
  ctx.window._ptfLastSavedCustCd = 'CUST-1002';
  ctx.window.saveCust2('CUST-1002');
  T('۲.۱۱ ویرایش صریح همان cd مهرشده همچنان تایید مجدد تک‌رکوردی می‌شود',
...
T('۴.۱ VERSION.json = v34.39.40', ver.crm_version === 'v34.39.40', ver.crm_version);
...
T('۴.۴ قرارداد UI/sw = 34.39.40',
  /window\.PTF_CRM_RELEASE = 'v34\.39\.40'/.test(idx) &&
  /CACHE = 'ptf-crm-v34\.39\.40'/.test(read('crm/sw.js')));
