...
T('ptfAuthSessionStore: نوشتن SS با try/catch و fallback به LS', (function () { var i = rb.indexOf('function ptfAuthSessionStore(sess)'); var seg = rb.slice(i, rb.indexOf('function ptfAuthLoginWrite')); return seg.indexOf("sessionStorage.setItem('ptf_crm_session', raw)") > -1 && seg.indexOf("localStorage.setItem('ptf_crm_session', raw)") > -1 && seg.indexOf('PTF_SESS_MEM.session = sess || null;') > -1; })()); /* v34.8.50: سه‌طبقه */
T('ptfAuthSessionStore: پاک‌کردن LS فقط بعد از خواندن-بازِ SS (v34.8.50)', (function () { var i = rb.indexOf('function ptfAuthSessionStore(sess)'); var seg = rb.slice(i, rb.indexOf('function ptfAuthLoginWrite')); var iV = seg.indexOf("!== raw) throw new Error('phantom')"); var iRm = seg.indexOf("localStorage.removeItem('ptf_crm_session')"); return iV > -1 && iRm > iV; })());
