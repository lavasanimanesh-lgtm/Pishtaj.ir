...
'use strict';
/* ═══════════════════════════════════════════════════════════════════════════
   tester591 — v34.37.6 — پنل‌های تاشوِ بالای فهرست (کارفرما ۲۰۲۶-۰۹-۰۴)
...
var VER = 'v34.37.6'; /* ابزار fix-version-drift این خط را خراب کرده بود (به‌سبب قاعدهٔ R-H که «var VER = 'v» را بدون نگاه‌داشتن به زمینهٔ regex بازنویسی می‌کرد) — در این فایل VER یک مقدار لفظیِ عامد است و window هم در Node وجود ندارد. */
...
/* v34.37.6: شکلِ escape‌شدهٔ نسخهٔ جاری برای پین‌هایی که با new RegExp('…') ساخته می‌شوند
...
T('۰.۲ cache-bust هر سه فایل در index.html', ['supplier-finance.js', 'scoring.js', 'customer-finance.js'].every(function (m) {
  /* v34.37.6: این پین به‌صورت رشتهٔ new RegExp('…') نوشته شده، پس escape آن «دوبل» است
     (34\\.37\\.2) و ابزارِ bump-version-pins پیش‌تر فقط شکلِ تک‌escape را می‌دید — یکی از
     دو نقطهٔ کوری که در v34.37.6 در خودِ ابزار رفع شد. */
  return new RegExp(m.replace('.', '\\.') + '\\?v=' + CUR_ESC2).test(idx);
}));
/* v34.37.6: نامِ *فایلِ* این تستر یک «هویتِ تاریخی» است، نه پینِ نسخه — ابزارِ bump
