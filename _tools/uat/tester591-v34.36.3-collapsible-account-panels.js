...
'use strict';
/* ═══════════════════════════════════════════════════════════════════════════
   tester591 — v34.37.0 — پنل‌های تاشوِ بالای فهرست (کارفرما ۲۰۲۶-۰۹-۰۴)
...
/* v34.37.0: شکلِ escape‌شدهٔ نسخهٔ جاری برای پین‌هایی که با new RegExp('…') ساخته می‌شوند
   (در متنِ فایل به‌صورت 34\\.37\\.0 دیده می‌شود). از VER مشتق می‌شود، نه لفظیِ دستی —
...
T('۰.۲ cache-bust هر سه فایل در index.html', ['supplier-finance.js', 'scoring.js', 'customer-finance.js'].every(function (m) {
  /* v34.37.0: این پین به‌صورت رشتهٔ new RegExp('…') نوشته شده، پس escape آن «دوبل» است
     (34\\.37\\.0) و ابزارِ bump-version-pins پیش‌تر فقط شکلِ تک‌escape را می‌دید — یکی از
     دو نقطهٔ کوری که در v34.37.0 در خودِ ابزار رفع شد. */
  return new RegExp(m.replace('.', '\\.') + '\\?v=' + CUR_ESC2).test(idx);
}));
/* v34.37.0: نامِ *فایلِ* این تستر یک «هویتِ تاریخی» است، نه پینِ نسخه — ابزارِ bump
