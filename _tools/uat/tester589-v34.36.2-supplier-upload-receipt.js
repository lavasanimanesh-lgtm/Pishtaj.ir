...
(async function () {
  ...
  await (async function () {
    ...
    T('۵.۴ فیلد ورودی فایل و accept آن دست‌نخورده', supSrc.indexOf('name="attachment" accept=".pdf,.doc,.docx,.xls,.xlsx,.zip,.rar"') > -1);
  ...
  T('۶ب.۱ اگر فایل اعلام شده ولی پیوست null است، خطا پر می‌شود (سکوت ممنوع)', AS.indexOf('$supDeclFile') > -1 && AS.indexOf("if ($attachment === null && $supDeclFile && $attachmentError === '')") > -1);
  ...
  T('۶ج.۳ هرگز پیوست موجود را بازنویسی نمی‌کند', /SUP-ATTACH-RECOVERY[\s\S]{0,900}empty\(\$dupFound\['row'\]\['attachment'\]\)/.test(api));
  T('۶ج.۴ فقط برای رکورد pending/rejected سایت (نه فهرست تاییدشدهٔ CRM)', /SUP-ATTACH-RECOVERY[\s\S]{0,900}\$dupFound\['list'\] === 'suppliers'[\s\S]{0,300}\['pending', 'rejected'\]/.test(api));
  T('۶ج.۵ هویت با تطابق شمارهٔ تماس (نرمال‌شده) بررسی می‌شود', /SUP-ATTACH-RECOVERY[\s\S]{0,1200}ptf_dedup_phone\(\$dupFound\['row'\]\['phone'\] \?\? ''\) === \$supPhoneNorm/.test(api));
