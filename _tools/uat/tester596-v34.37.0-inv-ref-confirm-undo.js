#!/usr/bin/env node
'use strict';
/* ═══ tester596 — v34.37.4 (INV-REF-CONFIRM + INV-REF-UNDO) ═══
...
T('۷.۵ خطای invoice_exists با نام فاکتورها به کاربر توضیح داده می‌شود',
  /* v34.37.4: میانِ سرِ تابع و alert یک بلوک «اعمال محلیِ فوری» اضافه شده — پنجره باز شد. */
  /ptfRevokeInvoiceRef=function[\s\S]{0,4800}dependencies[\s\S]{0,400}ابتدا فاکتور را ابطال کنید/.test(sdClient));
/* v34.37.4: پنل زندهٔ فاکتورها official-invoice-v2.js است (rbac.js بازنویسی می‌شود).
   دکمه در v34.37.4 اشتباهاً در کد مردهٔ rbac.js گذاشته شده بود و دیده نمی‌شد. */
...
console.log('\n— tester596 (v34.37.4: تایید پیش از ارجاع فاکتور + لغو ارجاع) —');
