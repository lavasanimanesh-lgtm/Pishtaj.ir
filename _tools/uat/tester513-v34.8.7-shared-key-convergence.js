#!/usr/bin/env node
'use strict';
/* v34.8.36 — SHARED-KEY-CONVERGENCE + فاز B CONFLICT-RESCUE.
...
T('avatars merge نسخه‌دار است (ts جدیدتر برنده؛ v34.8.36)', /strcmp\(\(string\)\$tsOf\(\$iv\), \(string\)\$tsOf\(\$sv\)\) >= 0/.test(api));
...
T('کلیدهای محافظت‌شده از حل‌کنندهٔ عمومی عبور نمی‌کنند (مسیر اختصاصی v34.8.36)', !/protectedKeys\.indexOf\(k\) >= 0\) return;/.test(cs) && /ptfSyncResolveProtectedConflictFromServer\(k, srvStr, payload\[k\]\)/.test(cs) && /else if \(typeof window\.ptfSyncResolveConflictFromServer === 'function'/.test(cs));
...
/* ---------- v34.8.36: ردیف صف phantom + merge نقشه‌ها در پول فاز B ---------- */
...
console.log('\n— tester513 (v34.8.36: همگرایی کلیدهای مشترک + نجات تعارض فاز B) —');
