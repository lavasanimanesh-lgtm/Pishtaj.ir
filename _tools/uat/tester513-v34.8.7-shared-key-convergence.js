#!/usr/bin/env node
'use strict';
/* v34.37.6 — SHARED-KEY-CONVERGENCE + فاز B CONFLICT-RESCUE.
...
T('avatars merge نسخه‌دار است (ts جدیدتر برنده؛ v34.37.6)', /strcmp\(\(string\)\$tsOf\(\$iv\), \(string\)\$tsOf\(\$sv\)\) >= 0/.test(api));
...
/* v34.37.6 (WATERMARK-ACK/R3): ثبت watermark از شاخهٔ نجات به helper مشترک
...
/* v34.37.6: فقط کلیدهای واقعاً ACK‌شده (savedKeys) جلو می‌روند — کلیدِ در تعارض/ردشده
   که watermark‌اش جلو برود، push بعدی دیگر conflict نمی‌دهد و کپی محلی نسخهٔ سرور را
   کورکورانه بازنویسی می‌کند. */
T('flush watermark کلید را از krevs پاسخ تازه می‌کند (helper مشترک v34.37.6)', /bAdvanceWatermarksFromPush\(result\.krevs, result\.rev, saved\)/.test(cs) && /function bAdvanceWatermarksFromPush\(krevsPlain, globalRev, ackedKeys\)/.test(cs) && /if \(onlyAcked && ackedKeys\.indexOf\(k\) < 0\) return;/.test(cs) && /if \(Object\.keys\(meta\)\.length\) bSaveRevsFromMeta\(meta, globalRev\);/.test(cs) && /bSaveRevsFromMeta\(metaLike, result && result\.rev\)/.test(cs));
T('watermark کلیدهای union پس از ACK جلو نمی‌رود (v34.37.6)', /window\.ptfSyncIsSharedUnionKey\(k\)\) return;/.test(cs) && /window\.ptfSyncIsSharedUnionKey = isSharedUnionKey;/.test(sync));
T('flush مجدد سقف‌دار است (حداکثر ۳ نوبت)', /flushRescueRound \|\| 0\) < 3/.test(cs));
T('کلیدهای محافظت‌شده از حل‌کنندهٔ عمومی عبور نمی‌کنند (مسیر اختصاصی v34.37.6)', !/protectedKeys\.indexOf\(k\) >= 0\) return;/.test(cs) && /ptfSyncResolveProtectedConflictFromServer\(k, srvStr, \(payload \|\| \{\}\)\[k\]\)/.test(cs) && /else if \(typeof window\.ptfSyncResolveConflictFromServer === 'function'/.test(cs));
/* v34.37.6 (R5): بدون عبور protectedConflicts از ptfBPushBatch، شاخهٔ بالا کدِ مرده بود. */
T('protectedConflicts از ptfBPushBatch به مصرف‌کننده می‌رسد (ریشهٔ بن‌بست v34.37.6)', /addUnique\(protectedConflicts, dProtected\);/.test(cs) && /protectedConflicts: protectedConflicts/.test(cs));
...
/* ---------- v34.37.6: ردیف صف phantom + merge نقشه‌ها در پول فاز B ---------- */
...
console.log('\n— tester513 (v34.37.6: همگرایی کلیدهای مشترک + نجات تعارض فاز B) —');
