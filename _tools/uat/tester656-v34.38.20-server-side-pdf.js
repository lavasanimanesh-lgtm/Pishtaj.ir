/* tester656 — v34.39.8 (ADV-CV-SERVER-SIDE-PDF-001):
...
console.log('── PDF باینری سمت سرور برای گزارش نهایی ADV-CV (v34.39.8) ──');
...
assert.ok(idx.indexOf('tool-report-drafts.js?v=34.39.8') > -1, 'cache-bust فایل UI اعمال شده');
...
if (!phpOk || fs.existsSync(secretsPath)) {
  ...
  (async function () {
    ...
    if (process.exitCode) {
      console.log('FAIL tester656 v34.39.8 server-side-pdf');
      process.exit(1);
    }
    console.log('PASS tester656 v34.39.8 server-side-pdf');
