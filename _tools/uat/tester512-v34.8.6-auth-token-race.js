#!/usr/bin/env node
'use strict';
/* v34.29.1 — AUTH-TOKEN-RACE / AUTH-LOGOUT-REVOKE / AUTH-401-HYGIENE.
...
T('index PTF_CRM_RELEASE = v34.29.1 (با کوتیشن)', /window\.PTF_CRM_RELEASE = 'v34\.29.1'/.test(idx));
T('sw.js RELEASE/CACHE = v34.29.1', /RELEASE = 'v34\.29.1'/.test(sw) && /CACHE = 'ptf-crm-v34\.29.1'/.test(sw));
T('phase-query موقتی حذف شد', !/f0[0-9]=20260825/.test(idx) && !/PHASE0[0-9]_QUERY/.test(sw));
T('همهٔ scriptها ?v یکسان با نسخهٔ رسمی', (function () { var re = /\?v=(\d+\.\d+\.\d+)/g, m, bad = 0; while ((m = re.exec(idx))) if (m[1] !== ver.crm_version.slice(1)) bad++; return bad === 0 && idx.indexOf('?v=' + ver.crm_version.slice(1)) > -1; })()); /* v34.29.1: چک پیشوندی قدیمی با نسخهٔ ۵۰ تصادم می‌کرد */
...
console.log('\n— tester512 (v34.29.1: ریشه‌یابی حلقهٔ «توکن معتبر وجود ندارد») —');
