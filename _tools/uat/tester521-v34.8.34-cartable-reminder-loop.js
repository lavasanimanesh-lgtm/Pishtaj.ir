#!/usr/bin/env node
'use strict';
/* tester521 — v34.37.6 (CARTABLE-LOOP): پایان حلقهٔ «یادآور در کارتابل هر چند ثانیه
   تکرار می‌شود».
...
T('state ضدتکرار از مسیر فرمان ذخیره می‌شود (نه پوش انبوه؛ v34.37.6: پچ مینیمال cd/notifiedUsers/msgSent — کل ردیف کهنه دیگر نمی‌رود)', /ptfEntityUpsert\('ptf_crm_reminders', \{ cd: pr\.cd, notifiedUsers: pr\.notifiedUsers \|\| \{\}, msgSent: !!pr\.msgSent \}/.test(br) && /else setData\('ptf_crm_reminders', rems\)/.test(br));
...
console.log('\n— tester521 (v34.37.6: CARTABLE-LOOP) —');
