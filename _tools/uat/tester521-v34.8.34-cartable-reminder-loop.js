#!/usr/bin/env node
'use strict';
/* tester521 — v34.37.5 (CARTABLE-LOOP): پایان حلقهٔ «یادآور در کارتابل هر چند ثانیه
   تکرار می‌شود».
...
T('state ضدتکرار از مسیر فرمان ذخیره می‌شود (نه پوش انبوه؛ v34.37.5: پچ مینیمال cd/notifiedUsers/msgSent — کل ردیف کهنه دیگر نمی‌رود)', /ptfEntityUpsert\('ptf_crm_reminders', \{ cd: pr\.cd, notifiedUsers: pr\.notifiedUsers \|\| \{\}, msgSent: !!pr\.msgSent \}/.test(br) && /else setData\('ptf_crm_reminders', rems\)/.test(br));
...
T('قرارداد نسخهٔ UI/sw = 34.37.5', /window\.PTF_CRM_RELEASE = 'v34\.37\.5'/.test(read('crm/index.html')) && /CACHE = 'ptf-crm-v34\.37\.5'/.test(read('crm/sw.js')));

console.log('\n— tester521 (v34.37.5: CARTABLE-LOOP) —');
