#!/usr/bin/env node
'use strict';
/* v34.37.4 — PROTECTED-CONFLICT-RESCUE.
...
T('v34.37.4: پذیرش verbatim کانونیکال وقتی لوکال از لحظهٔ ارسال تغییر نکرده', /sameSyncJson\(current, submittedStr\)\) \{\s*\n\s*merged = serverStr;/.test(sync));
T('v34.37.4: مسیر protected دیگر پاس tombstone ندارد (پایداری امضا)', (function () {
...
/* v34.37.4: منطق نجات به helper مشترک rescueConflictedKeys رفت تا «انتقال یک‌باره»
...
T('مسیر مهاجرت هم همان helper نجات را صدا می‌زند (v34.37.4)', /var rescued = rescueConflictedKeys\(d, payload\);/.test(cs));
...
(function behavior() {
  ...
  /* v34.37.4: adopt canonical → next push identical signature → saved */
...
console.log('\n— tester515 (v34.37.4: نجات تعارض کلیدهای مالی محافظت‌شده در فاز B) —');
