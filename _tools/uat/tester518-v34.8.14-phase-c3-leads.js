#!/usr/bin/env node
'use strict';
/* v34.26.1 — PHASE-C3 گام۱: تکمیل یادآورها (ثبت) + مهاجرت سرنخ‌ها به فرمان سروری.
...
T('sanitizer لیست اسکالر را نگه می‌دارد', /is_string\(\$item\)\) \{\s*\$stored = sd_text\(\$item, 300\);/.test(api)); /* v34.26.1: بدنه به sd_entity_sanitize_list منتقل شد */
T('sanitizer لیست نقشهٔ اسکالر (hist) را نگه می‌دارد', /subItem\[\$k3\] = \$storedSub;/.test(api));
T('SD_SERVICE_VERSION = 34.26.1', /SD_SERVICE_VERSION = '34\.26.1'/.test(api));

/* ---------- کلاینت ---------- */
T('پرچم سرنخ‌ها فعال شد', /'ptf_crm_leads': true/.test(v2)); /* v34.26.1: نقشه از این پس با W1 ادامه دارد — پرچم leads کافی است */
...
console.log('\n— tester518 (v34.26.1: PHASE-C3 گام۱ — یادآور کامل + سرنخ‌ها) —');
