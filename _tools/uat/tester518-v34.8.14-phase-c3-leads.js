#!/usr/bin/env node
'use strict';
/* v34.8.14 — PHASE-C3 گام۱: تکمیل یادآورها (ثبت) + مهاجرت سرنخ‌ها به فرمان سروری.
...
T('sanitizer لیست نقشهٔ اسکالر (hist) را نگه می‌دارد', /subItem\[\$k3\] = is_string\(\$v3\) \? sd_text\(\$v3, 500\)/.test(api));
T('SD_SERVICE_VERSION = 34.8.14', /SD_SERVICE_VERSION = '34\.8\.14'/.test(api));

/* ---------- کلاینت ---------- */
T('پرچم سرنخ‌ها فعال شد', /'ptf_crm_leads': true \}/.test(v2));
...
console.log('\n— tester518 (v34.8.14: PHASE-C3 گام۱ — یادآور کامل + سرنخ‌ها) —');
