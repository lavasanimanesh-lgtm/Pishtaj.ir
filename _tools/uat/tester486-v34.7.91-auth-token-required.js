#!/usr/bin/env node
'use strict';
/* v34.29.5 — ورود اجباری با توکن سرور (AUTH-TOKEN-REQUIRED).
...
T('inqreader.js cache-bust 34.29.5', /inqreader\.js\?v=34\.29.5/.test(idx));
...
T('ورود موفق همچنان session+token می‌سازد (از v34.29.5/R5: از طریق ptfAuthLoginWrite → sessionStorage؛ LS فقط پاک می‌شود)', idx.indexOf("ptfAuthLoginWrite(_login.token") > -1 && read('crm/rbac.js').indexOf("sessionStorage.setItem('ptf_crm_token'") > -1);
T('مسیر direct auth_login (users_get قدیمی) حفظ شده (از v34.29.5/R5: ptfAuthLoginWrite)', idx.indexOf("_directLogin") > -1 && idx.indexOf("ptfAuthLoginWrite(_directLogin.token") > -1);
...
console.log('\n— tester486 (v34.29.5: ورود اجباری توکن سرور — AUTH-TOKEN-REQUIRED) —');
