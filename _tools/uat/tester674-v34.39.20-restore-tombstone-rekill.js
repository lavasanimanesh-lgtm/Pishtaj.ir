'use strict';
/* ─────────────────────────────────────────────────────────────────────────────
   tester674 — v34.39.31 (RESTORE-TOMBSTONE-REKILL)
...
/* ── پورت وفادار لایهٔ سرور (api/crm.php sync_apply_tombstones + entity_*) ──
   ...
   v34.39.31 (قرینهٔ v34.37.0 ③) برگردانده شده و با grep های S7/S8 روی هر دو فایل پین است. */
...
function portApplyTombstones(key, json, serverArchiveJson, incomingArchiveJson) {
  ...
  if (key === 'ptf_crm_deleted_archive') {
    /* v34.39.31: alias-strip فقط برای پاک‌سازی گراف (archive_purge بدون collection)؛
...
setTimeout(function () {
  ...
  setTimeout(function () {
    ...
    console.log('\n── پین‌های انتشار ──');
    test('P1: نسخهٔ انتشار v34.39.31 است', version === 'v34.39.31', version);
    test('P2: Service Worker / index.html بامپ شده‌اند',
      sw.indexOf('v34.39.31') > -1 && indexHtml.indexOf("PTF_CRM_RELEASE = 'v34.39.31'") > -1);
    test('P3: تستر در گیت CI ثبت است', gate.indexOf('tester674-v34.39.20-restore-tombstone-rekill.js') > -1);
    test('P4: پین tester621 به نسخهٔ جدید به‌روز شده', tester621Src.indexOf("'v34.39.31'") > -1 && tester621Src.indexOf('v34.39.31') > -1);

    console.log('');
    if (failures) { console.log('=== tester674: ' + failures + ' FAIL ==='); process.exit(1); }
    console.log('PASS tester674-v34.39.31-restore-tombstone-rekill');
  }, 30);
}, 30);
