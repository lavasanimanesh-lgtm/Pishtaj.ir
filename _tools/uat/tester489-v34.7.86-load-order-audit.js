#!/usr/bin/env node
'use strict';
/* v34.7.86 — پایه‌ی «استارت سرد SPA» / سوادِ لود (LOAD-AUDIT-001، فقط استاتیک، بدون تغییر رفتار):
...
var wrongVer = localSrcs.filter(function (s) { return /\?v=/.test(s) && !/v=34\.7\.86/.test(s); });
T('همه‌ی cache-bust ها = 34.7.86', wrongVer.length === 0, wrongVer.join(', '));
...
T('tester489 در گیت CI', read('_tools/uat/run-ci-gate.js').indexOf('tester489-v34.7.86-load-order-audit.js') > -1);

console.log('\n— tester489 (v34.7.86: پایه‌ی استارت سرد — LOAD-AUDIT-001) —');
