#!/usr/bin/env node
'use strict';
/* v34.8.14 — PHASE-C2: زیرساخت فرمان عمومی موجودیت (مسیر نازک نوشتن).
...
T('SD_SERVICE_VERSION = 34.8.14', /SD_SERVICE_VERSION = '34\.8\.14'/.test(api));
...
T('اعمال projection بدون dirty (ptfBApplyServerProjection)', /entityApplyProjection[\s\S]{0,200}ptfBApplyServerProjection/.test(v2));
...
/* ---------- پایلوت leads.js ---------- */
T('remDone از فرمان سروری می‌گذرد', /function remDone[\s\S]{0,700}ptfEntityUpsert\('ptf_crm_reminders'/.test(leads));
T('remSnooze از فرمان سروری می‌گذرد', /function remSnooze[\s\S]{0,900}ptfEntityUpsert\('ptf_crm_reminders'/.test(leads));
...
console.log('\n— tester517 (v34.8.14: PHASE-C2 فرمان عمومی موجودیت) —');
