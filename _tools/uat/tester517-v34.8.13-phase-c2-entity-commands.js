#!/usr/bin/env node
'use strict';
/* v34.31.0 — PHASE-C2: زیرساخت فرمان عمومی موجودیت (مسیر نازک نوشتن).
...
/* ---------- پایلوت leads.js ---------- */
T('remDone از فرمان سروری می‌گذرد (v34.31.0: router یکپارچه SaveCollection = فرمان اتمیک + نوشتن محلی بی‌درنگ)', /function remDone[\s\S]{0,700}ptfEntitySaveCollection\('ptf_crm_reminders', rems, \{ reason: 'rem-done' \}\)/.test(leads));
T('remSnooze از فرمان سروری می‌گذرد (v34.31.0: همان router)', /function remSnooze[\s\S]{0,900}ptfEntitySaveCollection\('ptf_crm_reminders', rems, \{ reason: 'rem-snooze' \}\)/.test(leads));
...
console.log('\n— tester517 (v34.31.0: PHASE-C2 فرمان عمومی موجودیت) —');
