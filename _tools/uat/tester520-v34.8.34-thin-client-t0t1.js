#!/usr/bin/env node
'use strict';
/* tester520 — v34.15.0 (T0/T1 قراردادهای رودمپ نازک‌سازی):
...
T('A10 فهرست‌سفید لایهٔ داده دارد', /WHITELIST\s*=\s*\['client-server\.js',\s*'storage-quota\.js',\s*'sync\.js',\s*'storage\.js',\s*'backup\.js',\s*'rbac\.js'/.test(guard)); /* v34.15.0: +cheque-print (فلگ‌های رسانه) مجاز */
...
(function behavior() {
  ...
  T('پیشوند per-user دسته DEV', sandbox.window.ptfKeyCategory('ptf_tour_done_ali') === 'UI' || sandbox.window.ptfKeyCategory('ptf_autodraft_offer_CO') === 'DEV'); /* v34.15.0: personal_cheques از DEV به REC/سینک منتقل شد */
...
T('نسخهٔ UI = v34.15.0', /window\.PTF_CRM_RELEASE = 'v34\.15.0'/.test(idx));
var swv = read('crm/sw.js');
T('قرارداد sw/index هم‌نسخه', /CACHE = 'ptf-crm-v34\.15.0'/.test(swv) && idx.indexOf('?v=34.15.0') > -1);

console.log('\n— tester520 (v34.15.0: T0/T1 thin-client contracts) —');
