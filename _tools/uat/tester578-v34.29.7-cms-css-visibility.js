#!/usr/bin/env node
'use strict';
/* tester578 — v34.34.0: ریشه‌کنی واقعی «تب‌های خالی» مدیریت سایت — برخورد نام کلاس CSS
...
T('FIX: PTF_CMS_JS_VER = v34.34.0 (سپر نسخهٔ کهنه، همسو با VER پوسته)', cms.indexOf("window.PTF_CMS_JS_VER = 'v34.34.0';") > -1);
...
T('VER: VERSION.json = v34.34.0', JSON.parse(read('VERSION.json')).crm_version === 'v34.34.0');
T('VER: یادداشت انتشار نسخهٔ جاری موجود است', (function () { try { var cv = JSON.parse(read('VERSION.json')).crm_version; return fs.existsSync(path.join(ROOT, 'RELEASE-NOTES-' + cv + '.md')); } catch (e) { return false; } })()); /* v34.34.0: متا-چک نسخه-آگnostic — کلیدواژهٔ RCA مال نسخهٔ خودِ تستر است */
