#!/usr/bin/env node
'use strict';
/* tester556 — v34.38.0 (فاز S2 سئو): مولد صفحات
...
T('S2: اسکیمای Service بدون قیمت / Product + Offer فقط با قیمت مثبت (v34.38.19: Product بدون قیمت از نظر گوگل invalid است)', /case 'product_create'[\s\S]{0,9000}'@type' => \(\$price > 0 \? 'Product' : 'Service'\)[\s\S]{0,500}if \(\$price > 0\)[\s\S]{0,200}'offers'/.test(cmsPhp));
...
/* v34.38.0: فاصله‌محور نبود — سنجهٔ ترتیبیِ داخل بدنهٔ تابع (قرارداد نگهبان: «سنجه مبتنی بر فاصله کاراکتری در فایل فارسی‌دار غلط است») */
T('UI: ثبت خودکار نقشه پس از انتشار محصول', (() => { const i = cmsJs.indexOf('window.cmsProdPublish'); const j = cmsJs.indexOf('\n  window.', i + 30); const body = i > -1 ? cmsJs.slice(i, j > -1 ? j : undefined) : ''; return body.includes('cmsSitemapAfterPublish()'); })());
