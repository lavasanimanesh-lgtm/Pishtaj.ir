#!/usr/bin/env node
'use strict';
/* tester575 — v34.29.2: جستجوی فهرست محصولات CMS + عکس پیش‌فرض هوشمند + اصلاح لینک‌های
...
T('PHP: cta/footer محصول با cms_rel_links_fix اصلاح می‌شوند (لینک مرده → knowledge-center)', php.indexOf('function cms_rel_links_fix') > -1 && php.indexOf("cms_rel_links_fix($sk['cta']") > -1 && php.indexOf("cms_rel_links_fix($sk['footer']") > -1); /* v34.38.0: استخراج با هُلپر مقاوم */
...
['gate-valve-16-inch-cl600.html', '42-inch-wpb-wphy-elbow.html'].forEach(function (sl) {
  ...
  T('PAGE: products/' + sl + ' در قالب استاندارد (هیرو با عکس + اسکیما + بدون مسیرعمیق)', ok &&
    ...
    /"@type"\s*:\s*"Service"/.test(s) && /"@type"\s*:\s*"FAQPage"/.test(s) &&
    s.indexOf('../../') === -1);
});
