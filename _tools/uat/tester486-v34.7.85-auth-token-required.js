#!/usr/bin/env node
'use strict';
/* v34.7.85 — ورود اجباری با توکن سرور (AUTH-TOKEN-REQUIRED).
...
T('inqreader.js cache-bust 34.7.85', /inqreader\.js\?v=34\.7\.85/.test(idx));
...
T('ورود موفق همچنان session+token می‌سازد', idx.indexOf("localStorage.setItem('ptf_crm_token', _login.token)") > -1 && idx.indexOf("localStorage.setItem('ptf_crm_session'") > -1);
T('مسیر direct auth_login (users_get قدیمی) حفظ شده', idx.indexOf("_directLogin") > -1 && idx.indexOf("'ptf_crm_token', _directLogin.token") > -1);
...
T('tester486 در گیت CI', gate.indexOf('tester486-v34.7.85-auth-token-required.js') > -1);

console.log('\n— tester486 (v34.7.85: ورود اجباری توکن سرور — AUTH-TOKEN-REQUIRED) —');
