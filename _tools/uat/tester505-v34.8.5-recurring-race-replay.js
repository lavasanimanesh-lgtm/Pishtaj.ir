...
assert.ok(/in_array\(\$k, \['ptf_crm_opex','ptf_crm_sharetx'\], true\)/.test(crmApi));
assert.ok(/sync_merge_protected_finance_snapshot\(\$k, \$v, \$serverFinanceJson\)/.test(crmApi));
assert.ok(/Start from server so absence preserves every row/.test(crmApi));
...
assert.ok(/strtolower\(trim\(\(string\)\(\$server\['st'\].*=== 'settled'[\s\S]*\['st','settleDoc','settledBy','settledT','settleISO','payHow','acctTx'\][\s\S]*Financial evidence is append-only/.test(crmApi));
assert.ok(/OPXR-SRV-.*hash\('sha256', \$seed\)/.test(crmApi));
assert.ok(/\$merged\['serverOwnedIdentity'\] = true/.test(crmApi));
assert.ok(/\$incomingRowId !== ''[\s\S]*isset\(\$aliases\[\$rowAlias\]\)[\s\S]*empty\(\$row\['serverOwnedIdentity'\]\)/.test(crmApi));
...
assert.ok(/ptfSalesDomainCommand\('reconcile_shareholder_salaries'/.test(shareholders) && !/sharetx\.unshift|txs\.unshift/.test(shareholders));
