#!/usr/bin/env node
'use strict';
/* tester535 — v34.8.36: فیکس «کالاها نمایش داده نمی‌شوند».
...
T('v34.8.36: سرور خالی + دادهٔ محلی → رندر محلی (دادهٔ محلی حاکم تا sync)', /\(d\.total \|\| 0\) === 0 && localCount > 0/.test(idx));

var ver = JSON.parse(read('VERSION.json'));
T('VERSION.json = v34.8.36', ver.crm_version === 'v34.8.36', ver.crm_version);
T('قرارداد نسخهٔ UI/sw = 34.8.36', read('crm/index.html').indexOf("window.PTF_CRM_RELEASE = 'v34.8.36'") > -1 && read('crm/sw.js').indexOf("CACHE = 'ptf-crm-v34.8.36'") > -1);

console.log('\n— tester535 (v34.8.36: products read hardening) —');
