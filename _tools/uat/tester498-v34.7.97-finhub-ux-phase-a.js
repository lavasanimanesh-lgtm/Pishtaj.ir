#!/usr/bin/env node
'use strict';
/* tester498 — v34.7.99 (FINHUB-UX-A فاز A)
...
/* v34.7.99 (بازخورد کارفرما): نمایش کماکان به ریال — نه فشرده. اعداد کامل با
...
T('CSS .sc b سقف فونت 22px (v34.7.99: درشت‌تر برای خوانایی)', /\.sc\s+b\{[^}]*clamp\(16px,1\.5vw,22px\)/.test(idx));
T('CSS .sr minmax 180px (فضای کافی برای عدد ریالی بزرگ)', /\.sr\{[^}]*minmax\(180px,1fr\)/.test(idx));
T('CSS .sc padding 15px 13px (تنفس بیشتر)', /\.sc\{[^}]*padding:15px 13px/.test(idx));
T('CSS .sc .unit تعریف شده', /\.sc\s+b\s+\.unit\{/.test(idx));
T('CSS .sc .unit با 65% (متناسب با عدد درشت‌تر)', /\.sc\s+b\s+\.unit\{[^}]*font-size:65%/.test(idx));

/* ==== ۴) CSS موبایل (v34.7.99: درشت‌تر) ==== */
T('CSS موبایل: .sr minmax(160px)', idx.indexOf('minmax(160px, 1fr)') > -1);
T('CSS موبایل: .sc b font-size 16px (v34.7.99: از 13.5 → 16px)', /#panels\s+\.sc\s+b\s*\{[^}]*font-size:\s*16px/.test(idx));
T('CSS موبایل: .sc padding 12px 10px', /#panels\s+\.sc\s+\{[^}]*padding:\s*12px 10px/.test(idx));

/* ==== ۵) CSS دسکتاپ نوار تب @media(min-width:900px) — v34.7.99: chip افقی ==== */
T('CSS دسکتاپ: @media(min-width:900px) برای finHubBar',
  /@media\(min-width:900px\)\{[^}]*#panels\s+#finHubBar/.test(idx.replace(/\s+/g, ' ')));
T('CSS دسکتاپ v34.7.99: fin-hub-tab افقی (flex-direction:row)',
  /\.fin-hub-tab\{[^}]*flex-direction:row/.test(idx.replace(/\s+/g, ' ')));
T('CSS دسکتاپ v34.7.99: fin-hub-tab کوتاه (min-height:36px)',
...
console.log('\n— tester498 (v34.7.99: هاب مالی UX فاز A) —');
