...
head('۵. انطباق نسخه v34.38.15 و نگهبان گیت');

var verJson = JSON.parse(read('VERSION.json'));
var sw = read('crm/sw.js');
T('۵.۱ VERSION.json روی v34.38.15 است', verJson.crm_version === 'v34.38.15', verJson.crm_version);
T('۵.۲ شاخص رهاسازی index.html روی v34.38.15 است',
  idx.indexOf("window.PTF_CRM_RELEASE = 'v34.38.15'") > -1);
T('۵.۳ سرویس‌ورکر روی v34.38.15 است',
  sw.indexOf("RELEASE = 'v34.38.15'") > -1 && sw.indexOf("CACHE = 'ptf-crm-v34.38.15'") > -1);
T('۵.۴ مانیفست روی 34.38.15 است', JSON.parse(read('crm/manifest.json')).version === '34.38.15');
T('۵.۵ sales-domain.php روی 34.38.15 است',
  read('api/sales-domain.php').indexOf("const SD_SERVICE_VERSION = '34.38.15'") > -1);
var busts = idx.match(/\?v=([0-9.]+)/g) || [];
var badBust = busts.filter(function (b) { return b !== '?v=34.38.15'; });
T('۵.۶ همه cache-bust های index.html یکدست 34.38.15 هستند',
