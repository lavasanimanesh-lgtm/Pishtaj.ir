#!/usr/bin/env node
'use strict';
/* tester627 — v34.39.22 (SILENT-LOGIN + AUTO-MIGRATE-001 + LIQUID-RING)
   ...
   ⑥ نسخه‌ها هماهنگ‌اند (VERSION.json / manifest / sw.js / RELEASE / همهٔ
     queryهای ?v=34.39.22 در index.html) و داراییِ برش لوگو موجود است. */
...
assert.ok(idx.indexOf('window.PTF_CRM_RELEASE = \'v34.39.22\'') > -1, 'index: نسخهٔ ریلیز v34.39.22 است');
assert.ok(idx.indexOf('SILENT-LOGIN') > -1, 'index: یادداشت تغییرِ v34.39.22 آمده است');
...
assert.ok(vj.crm_version === 'v34.39.22', 'VERSION.json با v34.39.22 هماهنگ است');
assert.ok(read('crm/manifest.json').indexOf('"version": "34.39.22"') > -1, 'manifest.json با نسخهٔ جدید هماهنگ است');
assert.ok(sw.indexOf("var RELEASE = 'v34.39.22';") > -1 && sw.indexOf("var ASSET_VERSION = '34.39.22';")
  > -1 && sw.indexOf("var CACHE = 'ptf-crm-v34.39.22';") > -1,
  'sw.js: RELEASE/ASSET_VERSION/CACHE هر سه v34.39.22 هستند');
assert.ok(idx.indexOf('?v=34.38.16') === -1, 'index: هیچ query نسخهٔ قدیمی باقی نمانده است');
assert.ok(idx.indexOf('boot-splash.js?v=34.39.22') > -1 && idx.indexOf('sync.js?v=34.39.22') > -1
  && idx.indexOf('client-server.js?v=34.39.22') > -1,
...
console.log('PASS tester627-v34.39.22 SILENT-LOGIN + AUTO-MIGRATE-001 + LIQUID-RING');
