...
var SUITE = [
  ...
  { g: 'CRM (38.20): OFFER-INTEGRITY-DIAG — بن‌بستِ «کلید آفرز زرد»: data_push عمومی حق ایجاد پیشنهاد تازه را ندارد (فقط register_offer)؛ وقتی پیش‌نویسی روی دستگاه ثبت‌نشده می‌ماند سرور کل ptf_crm_offers را رد می‌کند و دکمهٔ ذخیره هم چون کلید dirty است قفل می‌شود. اصلاح ۱ (sales-domain-v2.js): اگر فقط ptf_crm_offers dirty است و پیش‌نویسِ بازِ فعلی روی سرور ثبت‌نشده، «ذخیره» همان پیش‌نویس را رسماً ثبت می‌کند؛ اصلاح ۲ (sync.js): پنل تشخیص علت را با نام نشان می‌دهد و دکمهٔ «بررسی پیشنهادهای ثبت‌نشده» می‌گذارد و «بررسی اتصال» دیگر خطای واقعی را بازنویسی نمی‌کند', f: '_tools/uat/tester652-v34.38.20-offer-integrity-diag.js' },
  { g: 'CRM (38.20): RFQ-APPROVE-VANISH — گزارش کارفرما «استعلام هوشمند سایت تایید شد ولی هیچ چیزی در درخواست‌ها یافت نمی‌شود»: rfqApprove وضعیت سایت را فوراً approved می‌کرد و درخواست از صندوق «در انتظار تایید» ناپدید می‌شد در حالی که درج رکورد ptf_crm_rfqs (entity_upsert) هنوز قطعی نبود؛ اگر فرمان رد می‌شد، رکورد فقط در silent-write محلی می‌ماند و اولین pull سرور آن را می‌شست. اصلاح ۱ (bridge.js): approve واقعی (set_status) فقط پس از ACK درج رکورد اجرا می‌شود و reason=site-approve (بدون حذف از پایهٔ کهنه)؛ در شکست درخواست در صندوق می‌ماند + خطای دقیق. اصلاح ۲ (bridge.js): صندوق، استعلام‌های «تاییدشدهٔ سایت ولی واردچرخه‌نشده» را جدا با دکمهٔ «ورود به چرخه» نشان می‌دهد تا رکوردهای گیرکردهٔ قبلی قابل بازیابی باشند', f: '_tools/uat/tester653-v34.38.20-rfq-approve-vanish.js' },
  { g: 'CRM/مالی (38.20): SH-SALARY-EDIT-PROPAGATE + SH-SALARY-ADVANCE — گزارش کارفرما: «حقوقِ یکی از کاربران را اصلاح کردم ولی در بخش حقوق تعهدیِ سال مالی تغییری ایجاد نشد؛ و مبلغی از حقوقِ یک سهامدار به‌صورت علی‌الحساب پرداخت شد ولی کسر نشد». ریشهٔ ۱: reconcile_shareholder_salaries فقط ماهِ ارسالی را با مبلغ جدید به‌روز می‌کرد و ماه‌های قبلی مبلغ قدیم می‌ماند. حالا ویرایشِ مبلغ حقوق سهامدار پرچم applySalaryAmountAllMonths می‌فرستد و سرور مبلغِ همهٔ ماه‌های بازِ حقوقِ همان سهامدار (مطالبهٔ sharetx + هزینهٔ OPEX پیوندخورده) را به مبلغ جدید می‌برد (ماهِ سال قفل‌شده رد، بدون void/ساخت، ثبت در corrections). ریشهٔ ۲: دکمهٔ «علی‌الحساب» همیشه draw بدون نشان حقوق می‌ساخت و در حقوق پرداخت‌شده شمرده نمی‌شد؛ حالا دیالوگِ برداشت انتخاب «علی‌الحساب حقوق» دارد که draw را با salaryMonth ثبت می‌کند ⇒ از حقوق تعهدیِ پرداخت‌نشده کم می‌شود و در علی‌الحساب سود نمی‌آید؛ برداشتِ عادی (بدون نشان) همچنان علی‌الحساب سود است (قرارداد tester628 دست‌نخورده)', f: '_tools/uat/tester654-v34.38.20-salary-edit-propagate-salary-advance.js' },
  { g: 'سایت/SEO (38.20): INDEX-TRACKER-REQUEST-LINK — گزارش کارفرما: «در ردیابِ ایندکسِ افزایشی در سرچ کنسول، برای هر ردیفی که ایندکس نشده باید دکمهٔ مستقیمِ «درخواست ایندکس» وجود داشته باشد»: جدولِ pending_list فقط «صفحه/وضعیت/گوگل می‌گوید» را نشان می‌داد و هیچ عملِ مستقیمی روی ردیف نبود. حالا سرور برای هر ردیفِ pending، inspectLink (صفحهٔ بازرسیِ سرچ کنسولِ همان نشانی) را از gsc_inspect_link برمی‌گرداند و کلاینت ستون «عملیات» با «🔎 بررسی» (بازرسیِ زنده) + «درخواست ایندکس ↗» (پیوند مستقیم به Request Indexing — چون Indexing API عمومی برای این صفحات وجود ندارد) اضافه می‌کند؛ قرارداد افزایشیِ ردیاب (tester637) دست‌نخورده است', f: '_tools/uat/tester655-v34.38.20-index-tracker-request-link.js' },
  { g: 'ابزارها/ADV-CV (38.21): SERVER-SIDE-PDF — تکمیل P1 بک‌لاگ: «PDF باینری سمت سرور». گزارش نهایی ADV-CV حالا علاوه بر HTML (Print/Save-as-PDF)، PDF باینری با wkhtmltopdf سمت سرور می‌سازد: رندر best-effort در لحظهٔ Issue (شکست مختل نمی‌کند)، تولید idempotent برای گزارش‌های پیشین (admin_report_final_pdf_generate)، استریم با احراز هویت + verify checksum (fail-closed) و وضعیت نصب در gate. امنیت: رندر در دایرکتوری خصوصی crm/data (Deny from all) با --allow محدود + نام sanitize‌شده. تست‌های رفتاری واقعی PHP (stub wkhtmltopdf) فقط در محیط بدون ptf-secrets.php واقعی اجرا می‌شوند', f: '_tools/uat/tester656-v34.38.20-server-side-pdf.js' },
  { g: 'CRM/مالی (38.22): RECEIPT-BUTTON-FIX — گزارش کارفرما: «دکمهٔ دریافت در پروندهٔ فروش کار نمی‌کند». ریشه: از v34.38.19 (INV-OPEN-CANONICAL) ستون «مطالبهٔ باز» جدول فاکتورهای پنجرهٔ «💳 دریافت و حساب پرونده» (ptfCaseFinanceOpen) به arCore ارجاع می‌داد که فقط داخل caseTotals تعریف شده بود ⇒ برای هر پروندهٔ دارای فاکتور ReferenceError و بازنشدنِ کامل پنجره (دکمه مرده؛ دقیقاً هنگام ثبت دریافتِ فاکتور). اصلاح (sales-domain-v2.js): تعریف var arCore=(window.PTF||{}).ar داخل خود ptfCaseFinanceOpen — همان الگوی caseTotals با fallback فرمول قدیم وقتی PTF.ar حاضر نیست. تستر۶۴۲ فقط regex ایستا بود؛ تستر۶۵۷ تابع واقعی را در vm با موتور PTF.ar اجرا می‌کند (۴ سناریو: دارای فاکتور، بدون فاکتور، بدون PTF.ar، ستون بازِ منبع‌واحد با وصولی میراثی)', f: '_tools/uat/tester657-v34.38.22-case-receipt-button-fix.js' },
  { g: 'CRM/مکاتبات (38.23): LETTER-SIG-PAGINATION — گزارش کارفرما: «در مکاتبات/ثبت نامهٔ صادره، مهر امضا به صفحهٔ بعد منتقل می‌شود و گاهاً یک صفحهٔ خالی بین متن و امضا وجود دارد». سه ریشه در letPrintObj (crm/letters.js): ① بلوک .sig بدون break-inside:avoid بین صفحات می‌شکست — نام/سمت در صفحهٔ قبل و مهر (absolute داخل sigbox) و تصویر امضا در صفحهٔ بعد؛ ② .body table{page-break-inside:avoid} برای جدول بلندتر از صفحه، باگ «صفحهٔ خالی» کروم را فعال می‌کرد؛ ③ پاراگراف/‌<br>‌های خالیِ انتهای متن (باقی‌ماندهٔ کپی Word) ارتفاع نامرئی اشغال و امضا را بی‌دلیل به صفحهٔ بعد هل می‌دادند. اصلاح: .sig/.sigbox اتمیک (قرارداد .endsig سربرگ و .sig پیشنهاد)؛ جدول با الگوی offers-pro (auto + tr:avoid + thead:header-group) در هر دو مسیر چاپ؛ letTrimTrailingEmptyHtml فقط برای انتهای بدنهٔ مسیر استاندارد (مسیر «متن آماده روی سربرگ» به قرارداد «بدون تغییر متن» دست نمی‌زند). تستر: اجرای واقعی letPrint در vm + واحدِ تابع پیرایش (۲۱ چک؛ روی کد قبل از اصلاح قرمز)', f: '_tools/uat/tester658-v34.38.23-letter-sig-pagination.js' },
  { g: 'CRM/اعلانات (38.24): NOTIF-FRESH — گزارش کارفرما: «اعلانات و کارتابل اصلاً به‌روز نیست؛ کارها/اعلانات بسیار قدیمی می‌مانند؛ خواندم موقت پاک می‌کند و پس از هارد رفرش همان اعلانات قدیمی برمی‌گردد؛ باید سیستمی طراحی شود که اعلانات مهم و به‌روز دیده شوند و با گزینهٔ خواندم از بین بروند». چهار ریشه: ① notify/addMsg کارتِ خوانده‌شده با dkey صریح را با cd تازه و readBy خالی بازتولید می‌کرد (حلقهٔ روزانهٔ chq-due/quota/mgmt-report) — حالا dedupe چسبنده: هویت پایدار = یک کارت تا حل منبع؛ ② کارت‌های اقدام‌دار هرگز پیر نمی‌شدند (v33.4.1) — سیاست سنی: خوانده‌نشدهٔ >۳۰ روز → done+ageArchived (خارج از نمای پیش‌فرض، suppress بازتولید)، خوانده‌شدهٔ >۶۰ روز و بایگانی >۹۰ روز → سخت حذف تک‌رکوردی؛ ③ pull برای notifs جایگزینی خام می‌کرد — readBy محلیِ commitنشده (فرمان entity_upsert در پرواز که با هارد رفرش گم می‌شد) پاک می‌شد و اعلان خوانده‌شده برمی‌گشت — حالا همیشه union merge (readBy/done/ageArchived) + tombstone پس از merge + dirty برای همگرایی؛ ④ ژورنال فرمان‌های در پرواز (pagehide → localStorage → بازپخش پس از bootstrap با همان idempotencyKey — WAL سرور اثر دوباره ندارد). همچنین: STALE-BOOT-GUARD روی چهار سازندهٔ کارت (RCA 2026-09-08 §7)، اتحاد readBy/done در entity_upsert سرور، اتحاد ageArchived در merge کانونیک data_push (crm.php + sales-domain.php). تستر۶۵۹: ۴۳ چک (ایستا+رفتاری؛ روی کد قبل از اصلاح ۲۲ FAIL)', f: '_tools/uat/tester659-v34.38.24-notif-fresh.js' },
  { g: 'CRM/مشتریان (38.25): CONTACT-STALE-HOOK-WIPE — گزارش کارفرما: «هر مشتری که مدیرعامل با شماره ثبت می‌کند رکورد می‌ماند ولی شماره‌ها برای من (رییس هیات مدیره) ناپدید است؛ ثبت‌های خودم سالم است». ریشهٔ اثبات‌شده با کد واقعی: هوک phonefmt حتی پس از ذخیرهٔ زودبرگشتی (بلاک ضدتکرار/نام خالی) «آخرین رکورد ذخیره‌شدهٔ تب» را از کش کهنه با expectCreate:false دوباره upsert می‌کرد و merge سرور «کلید حاضر بازنویسی» تغییرات دستگاه‌های دیگر (شماره/ایمیل اضافه‌شده) را می‌شست — دستگاه ثبت‌کننده از کش خودش نسخهٔ تماس‌دار را می‌دید (الگوی نقش‌نما بدون تبعیض نقشی). اصلاح سه‌لایه: ① نرمال‌سازی/قالب‌بندی شماره‌ها به داخل saveCust2/saveSup2 پیش از payload منتقل شد (حالت خارجی تامین‌کننده از فرم خوانده می‌شود — شاخهٔ EN هوک در عمل مرده بود)؛ ② هوک فقط روی ویرایش صریح تایید مجدد تک‌رکوردی می‌کند؛ ثبت جدید و ذخیرهٔ زودبرگشتی هیچ فرمان شبکه‌ای ندارند؛ ③ invariant سمت سرور CONTACT-WIPE در entity_upsert: «کلید حاضرِ خالی» تماس‌های ذخیره‌شده را نمی‌شوید مگر با مهر _ccClear از کلاینت جاری (پاک‌سازی عمدی؛ فیلد نیت پیش از ذخیره حذف می‌شود). قرارداد: tester660 (۳۷ چک؛ ایستا + گارد سرور با پورت PHP + زنجیرهٔ کامل سه‌دستگاهه با بوت واقعی) و بند ۲.۹ tester604 عمداً به قرارداد جدید به‌روز شد', f: '_tools/uat/tester660-v34.38.25-contact-stale-hook-wipe.js' },
  { g: 'سایت/صفحهٔ اول (38.25): HOME-MODERN-FAB-CHAT — گزارش کارفرما: «ظاهر خیلی مدرن‌تر و داینامیک‌تر و کاربرپسندتر به صفحهٔ اول با حفظ پرستیژ محتوا و تم شرکت؛ آیکون‌های چت/تماس/واتساپ بزرگ و غیرمتناسب‌اند؛ پنجرهٔ چت هوشمند زیر آیکون واتساپ باز می‌شود و مزاحمت می‌سازد». سه کار: ① FAB-CHAT-HARMONY (ptf-chat.js): z-index پنجرهٔ چت (۱۰۰۵۰) از واتساپ (۹۹۹۹) و تماس (۴۵) بالاتر؛ هنگام باز بودن چت با کلاس body.ptf-chat-open دکمه‌های تماس/واتساپ محو/غیرقابل‌کلیک می‌شوند؛ بستن با ESC؛ انیمیشن نرم بازشدن. ② نظام واحد دکمه‌های شناور: قطر یکنواخت ۴۸px (موبایل ۴۴px)، پشتهٔ چت ۲۰ ← تماس ۸۲ ← واتساپ ۱۴۴ با فاصلهٔ یکنواخت، آیکون svg متناسب (واتساپ ۲۳px)، hover یکدست و حذف هندلرهای درون‌خطی. ③ لایهٔ HOME-MODERN فقط-نمایشی در home.css: ورود سینمایی هیرو، هالهٔ زندهٔ اورلی، ورود پلکانی reveal، پولیش hover کارت‌های journey/why-ptf/stats/trust، درخشش دکمهٔ اصلی، مکث مارکی روی hover، نوار پیشرفت اسکرول (rAF passive) و prefers-reduced-motion؛ هیچ متن/هوک/اسکیمایی تغییر نمی‌کند (نگهبان‌های ۵.*tester661 + tester210/216/220/318 سبز می‌مانند). قرارداد: tester661 (۲۸ سنجه)', f: '_tools/uat/tester661-v34.38.25-home-modern-fab-chat.js' },
  { g: 'سایت/صفحهٔ اول (39.0): HOME-DYNAMIC-X — درخواست کارفرما: داینامیک‌ترین/زیباترین الگوی سایت‌های تامین تجهیزات + تم بهینه با وفاداری به رنگ لوگو + حداکثر سرعت + موبایل حرفه‌ای. بنچ‌مارک: Grainger/McMaster/Fastenal (search-first) + الگوهای B2B صنعتی ۲۰۲؛ تم: ۶/۳۰/۰ با سرمهٔ مهندسی + شعلهٔ برند. ۵ کار: ① finder تجهیز در هیرو (GET /search/?q= — بدون JS هم کار می‌کند)؛ ② داک پایین موبایل اپ‌گونه (۵ هدف لمسی ≥۵۴px، safe-area، جایگزینی شناورها) + بک‌دراپ/قفل اسکرول/✕ همبرگر؛ ③ تایپ‌کینتیک کلمات H1، اسپات‌لایت، شمارنده، هدر جمع‌شونده — ptf-motion.js صفر کتابخانه، passive، prefers-reduced-motion؛ ④ باند سرمهٔ سوابق، یکپارچه‌سازی آیکون‌ها با تم، theme-color دوحالته، viewport-fit؛ ⑤ content-visibility، مکث اسلایدر در تب پنهان، سوایپ لمسی هیرو. قرارداد: tester662 (۳۶ سنجه)', f: '_tools/uat/tester662-v34.39.0-home-dynamic-x.js' },
  { g: 'سایت/صفحهٔ اول (39.1): HOME-ELECTRIC-3D — دستور کارفرما: جریان برق روی کارت/دکمه‌ها + هاور رنگی + حذف منوی بالای موبایل و انتقال به شیت پایین با انیمیشن + روز/شب ماندگار (کوکی، ضدFOUC) + حذف RFQ از داک و منوی وسط سه‌بعدی + فلش‌های مینیمال + اعداد فارسی پنل هیرو. لایهٔ v34.39.1 در home.css (current-ring با mask، تیلت rAF، پالت شب کامل) + ptf-motion (شیت/تم/ارو/تیلت). قرارداد: tester663 (۳۰ سنجه)', f: '_tools/uat/tester663-v34.39.1-home-electric-3d.js' },
  ...
  { g: 'CRM/پرونده (39.20): RESTORE-TOMBSTONE-REKILL — گزارش کارفرما: «پرونده را بایگانی کردم، به جریان انداختم و به پرونده‌های فروش منتقل شد؛ بعد از مدتی پیشنهاد آن پرونده به‌عنوان پیشنهاد بدون پرونده نمایش داده شد و کلاً پرونده از داده‌ها پاک شده بود». ریشه با اجرای کد واقعی: بازگشت، رکورد را بدون تاریخ ساخت ISO می‌ساخت و خنثی‌سازی سنگ‌قبر (sfNeutralizeDealTombstones) هرگز به سرور نمی‌رسید (legacyFallback با ردیف‌های بی‌cd آرشیو حذف) → سنگ‌قبر archive_purge فعال می‌ماند و اولین data_push/data_pull با dirty پس از upsert ناموفق، پرونده را بی‌صدا می‌کشت؛ فیلتر alias زیررشته‌ای آرشیو، ردیف‌های restored:/recycle را هم می‌سوخت (حذف کامل بی‌اثر + orphan_won). رفع ریشه‌ای: فرمان اتمیک سروری entity_tombstones_neutralize (قرینهٔ گام ② entity_restore) + ذخیرهٔ سطری خنثی‌سازی روی _id + createdAt/createdAtISO روی رکورد بازگشت + diff روتر آرشیو با هویت _id + محافظت پروندهٔ دارای restoredFrom در برابر رکورد بایگانی زامبی (جاروی یکتایی) + پیوند offerNos در caseBelongsToOffer + فیلتر alias فقط برای پاک‌سازی گراف بدون collection (هم‌راستا با قرارداد v34.37.0 ③)', f: '_tools/uat/tester674-v34.39.20-restore-tombstone-rekill.js' },
  { g: 'CRM/UI (39.21): UI-STABILITY — گزارش کارفرما: «در پنجره ثبت قیمت در قسمت تامین اینقدر لغزش و پرش و ناپایداری وجود دارد / با جابه‌جایی روی تب‌ها بعضی چیزها مثل فیلتر بعدا لود می‌شوند و پرش می‌دهند». ریشه با اجرای کد واقعی: ① hint «مبلغ به حروف» moneyx داخل ردیف‌های flex مودال «💰 ثبت قیمت دور» به آیتم جریانی تبدیل می‌شد و با هر keystroke (رشد متن حروفی/تبدیل ارقام بی‌کرسر) فیلدها می‌لغزیدند + بعد از هر ثبت، kill-all مودال‌ها و destroy/recreate کل مودال مقایسه (cmpQuoteSave/cmpBuy/cmpSplitSave/cmpBulkBuyGo) + rfqsmart با blur فیلد قیمت کل آکاردئون را وسط تایپ بازسازی می‌کرد؛ ② listtools دکمهٔ «🧰 فیلتر و خروجی» را فقط با setInterval(1200) تزریق می‌کرد (فیلتر تا ۱٫۲s بعد از تعویض تب می‌آمد و می‌پرید) + باکس‌های تنظیمات با setTimeout 100-150ms پر می‌شدند. رفع ریشه‌ای: hint تک‌خطهٔ غیر-reflow (nowrap+ellipsis) در خط اختصاصی flex/grid + حفظ کرسر تبدیل ارقام + cmpInnerHtml مشترک و cmpRefreshModal با حفظ scrollTop (fallback به cmpOpen) + نگهبان containment آکاردئون در rfqsPriceBlur + تزریق هم‌زمان/پیش از paint دکمهٔ فیلتر (MutationObserver + ptfListToolsInject) + رزرو min-height باکس‌های تنظیمات', f: '_tools/uat/tester675-v34.39.21-ui-stability-price-modal-tabs.js' },,
  { g: 'CRM/تماس (39.22 + R7 39.30): CONTACT-ROOTS — شماره‌های تماس ثبت‌شده پاک یا نامرئی نشوند؛ R7: اتحاد رکوردبه‌رکوردِ تماس در data_push (sync_contact_records_merge + api/contact-merge-lib.php، هم‌سنجی با PHP واقعی روی CI)', f: '_tools/uat/tester676-v34.39.22-contact-roots.js' },
