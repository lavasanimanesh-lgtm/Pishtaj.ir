#!/usr/bin/env node
'use strict';
/* v34.7.70 — جلوگیری سروری از ثبت تکراری تامین‌کننده.
...
T('در برابر suppliers (pending) و ptf_crm_suppliers (تاییدشده)', crm.indexOf("array_merge(load_data('suppliers'), load_data('ptf_crm_suppliers'))") > -1);
...
T('رکورد تکراری ساخته نمی‌شود (duplicate قبل از save)', crm.indexOf("'error' => 'duplicate'") > -1 && crm.indexOf("'error' => 'duplicate'") < crm.indexOf("save_data('suppliers', $suppliers"));

T('tester472 در گیت CI', gate.indexOf('tester472-v34.7.70-sup-dedup.js') > -1);

console.log('\n— tester472 (v34.7.70: جلوگیری از ثبت تکراری تامین‌کننده) —');
