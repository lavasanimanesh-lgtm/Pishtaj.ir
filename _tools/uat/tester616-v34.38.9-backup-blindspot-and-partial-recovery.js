...
/* =============================================================================
   ...
   این تستر قرارداد اصلاح v34.38.15 را قفل می‌کند:
...
head('۴. بازیابی فیلد-به-فیلد (شستشوی جزئی) — قرارداد v34.38.15');
var R = bootRc({});
...
T('۷.۰ VERSION.json = v34.38.15', ver.crm_version === 'v34.38.15', ver.crm_version);
T('۷.۱ tester616 در run-ci-gate.js ثبت است', gate.indexOf('tester616-v34.38.9-backup-blindspot-and-partial-recovery.js') > -1);
T('۷.۲ قرارداد UI/sw/SD = 34.38.15',
  /window\.PTF_CRM_RELEASE = 'v34\.38\.15'/.test(read('crm/index.html')) &&
