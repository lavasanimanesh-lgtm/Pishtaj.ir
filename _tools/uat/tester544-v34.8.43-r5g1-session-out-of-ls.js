#!/usr/bin/env node
'use strict';
/* tester544 — v34.17.0 (R5/T4-1b — SESSION-OUT-OF-LS): توکن/نشست خارج از localStorage
...
T('مهاجرت: حذف LS فقط بعد از تأیید نگه‌داشتِ SS (v34.17.0)', /sessionStorage\.setItem\(_k, _v\);[\s\S]{0,120}if \(sessionStorage\.getItem\(_k\) !== _v\) continue;[\s\S]{0,80}localStorage\.removeItem\(_k\);/.test(rbac));
...
T('خوانش مستقیم LS توکن فقط در rbac.js (مهاجرت + خواندن-بازِ v34.17.0)', directReads.length === 1 && directReads[0] === 'rbac.js:2', JSON.stringify(directReads));
T('نوشتن LS توکن فقط fallback خرابی SS (هات‌فیکس v34.17.0)', jsFiles.every(function (x) {
...
function finish() {
  console.log('\n— tester544 (v34.17.0: R5/T4-1b — توکن/نشست خارج از localStorage؛ کوکی HttpOnly + آینهٔ sessionStorage) —');
