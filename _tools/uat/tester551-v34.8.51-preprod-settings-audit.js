...
/* tester551 — v34.8.51 (PRE-PROD): تنظیمات متناسب با وضعیت فعلی + آمادگی پروداکشن
   ...
   حذف موتور در v34.37.5 ناسازگار بود).
...
T('backup.js: پاک‌سازی کش فقط در سبزِ کامل؛ حالت ناتمام/منتقل‌نشده دکمهٔ انتقال دارند (v34.37.5 سه‌حالته)', (function () { var i = bak.indexOf('انتقال یک‌بارهٔ داده‌های این دستگاه'); var j = bak.indexOf('ptfBClearLocalCache()'); var g = bak.indexOf('var green = on && synced;'); var fin = bak.indexOf("'تکمیل انتقال یک‌باره'"); return i > -1 && j > -1 && g > -1 && fin > -1 && /\(green\s*\?\s*'[^']*ptfBClearLocalCache[^']*'\s*:\s*'[^']*ptfBConfirmFlush/.test(bak.slice(g - 100, g + 2400)); })());
