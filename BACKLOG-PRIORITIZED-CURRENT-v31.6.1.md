...
| P0 | `FIN-WF-004` | guard سال مالی پراکنده و ناقص است؛ مسیرهای invoice/customer/shareholder کامل پوشش ندارند | inventory همهٔ mutationها و guard مرکزی |
| P1 | `FIN-WF-006` | `savePay()` append می‌کند؛ reversal استاندارد در کد فعلی پیدا نشد | receipt void/reversal با audit |
...
| P1 | `US-435` | اتصال UI و predicate فعال مشتری در `v31.6.2` انجام شد؛ full regression هنوز fail است | targeted UAT و رفع failureهای معتبر پیش از ZIP Production |
...
| اولویت | شناسه | وضعیت جاری | اقدام بعدی |
|---|---|---|---|
| P0 | `BUG-PROC-LINK-287 / FIN-WF-019` | resolver و لایهٔ انتخاب خرید واقعی با provenance پایدار در `v31.6.2` تکمیل شد؛ دادهٔ legacy بدون provenance عمداً انتخاب نمی‌شود | UAT و بررسی موارد legacy مبهم، بدون migration خودکار |
...
| اولویت | شناسه | وضعیت جاری | اقدام بعدی |
|---|---|---|---|
| P1 | `US-437` | در `v31.6.1` rollback شده؛ فقط مستندات/کیس‌استادی | parked تا approval مستقل و طراحی workflow |
