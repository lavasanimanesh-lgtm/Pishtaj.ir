...
const SD_SERVICE_VERSION = '34.8.19';
...
function sd_entity_sanitize_row(array $row, array &$stats = null): array {
    /* v34.8.19 (T1-3): فیلد null حفظ می‌شود (یادآورها link:null می‌سازند)، سقف متن
    ...
    elseif ($action === 'entity_upsert' || $action === 'entity_delete') {
        ...
        if ($action === 'entity_upsert') {
            ...
            if ($found < 0) {
                ...
                /* v34.8.19 (CARTABLE-LOOP): merge semantics — فیلدی که در payload نیست
