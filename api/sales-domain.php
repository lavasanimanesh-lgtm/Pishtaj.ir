...
const SD_SERVICE_VERSION = '34.8.32';
...
function sd_entity_registry(): array {
    return [
        ...
        /* v34.8.32 (W1 — ROADMAP-THIN-CLIENT T2 موج اول، تأیید کارفرما):
        ...
        /* v34.8.32 (W2 — T2 موج دوم): درخواست‌ها/پرونده‌ها/پروژه‌ها + اقلام/پکینگ/کارتابل.
        ...
        /* v34.8.32 (W3 — T2 موج سوم، حساس‌ترین مالی): فاکتورها/رسیدها/تخصیص‌ها/اصلاحات/
        ...
        /* v34.8.32 (W4 — T2 موج چهارم، تکمیل کامل): همهٔ کلیدهای کسب‌وکار باقی‌مانده.
        ...
        /* v34.8.32 (T5-2): چک‌های شخصی از کلیدهای فقط-دستگاه (ptf_personal_cheques_<user>)
...
function sd_entity_sanitize_row(array $row, array &$stats = null, int $maxFields = 40): array {
    /* v34.8.32 (T1-3): فیلد null حفظ می‌شود (یادآورها link:null می‌سازند)، سقف متن
    ...
    elseif ($action === 'entity_upsert' || $action === 'entity_delete') {
        ...
        if ($action === 'entity_upsert') {
            ...
            if ($found < 0) {
                ...
                /* v34.8.32 (CARTABLE-LOOP): merge semantics — فیلدی که در payload نیست
        ...
        /* v34.8.32 (W2): کلیدهای sortIso (notifs) — آرایهٔ سرور iso نزولی بماند؛
