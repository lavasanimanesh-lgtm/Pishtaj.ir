...
| P1 | `FIN-WF-009` | مسیر settled به void و transaction معکوس وجود دارد؛ regression کامل v31.6.20 سبز است | verification مالی تکمیلی در staging/Production |
...
| اولویت | شناسه | وضعیت جاری | اقدام بعدی |
|---|---|---|---|
| P0 | `BUG-OPPO-SYNTAX` | در source رفع شد؛ `node --check`، `tester127` و regression کامل v31.6.20 سبز | نگه‌داشتن evidence و UAT مرورگر |
| P1 | `US-435` | اتصال UI و predicate فعال مشتری در `v31.6.20` انجام شد؛ regression کامل ۱۴۹ فایل و ۳۷۱۹ چک سبز است | targeted UAT مرورگر و نگه‌داشتن evidence |
...
| اولویت | شناسه | وضعیت جاری | اقدام بعدی |
|---|---|---|---|
| P0 | `BUG-PROC-LINK-287 / FIN-WF-019` | resolver و لایهٔ انتخاب خرید واقعی با provenance پایدار در `v31.6.20` تکمیل شد؛ دادهٔ legacy بدون provenance عمداً انتخاب نمی‌شود | UAT و بررسی موارد legacy مبهم، بدون migration خودکار |
| P0 | `REG-103` | بخش index-based در Optimizer اصلاح شد؛ fixture provenanceدار، تست targeted و regression کامل v31.6.20 سبز هستند | بررسی browser واقعی و موارد legacy مبهم |
...
| اولویت | شناسه | وضعیت جاری | اقدام بعدی |
|---|---|---|---|
| P1 | `US-437` | در `v31.6.20` rollback شده؛ فقط مستندات/کیس‌استادی | parked تا approval مستقل و طراحی workflow |
...
| P1 | `GOLIVE-RESET-001` | `ptf_crm_payables` و `ptf_crm_surplus` در reset تراکنشی نبودند؛ در v31.6.20 به `WIPE_KEYS` اضافه و tester174 سبز شد | verification غیرمخرب pre-golive روی staging |
...
| اولویت | شناسه | وضعیت جاری | اقدام بعدی |
|---|---|---|---|
| P0 | `REG-RUNNER-149/150/151` | parser runner فعلی هر دو الگوی خروجی را می‌خواند؛ full regression v31.6.20 سبز است | نگه‌داشتن regression gate؛ تغییر محصول لازم نیست |
