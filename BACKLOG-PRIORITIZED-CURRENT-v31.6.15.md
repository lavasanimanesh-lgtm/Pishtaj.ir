...
| P1 | `US-435` | اتصال UI و predicate فعال مشتری در `v31.6.13` انجام شد؛ full regression هنوز fail است | targeted UAT و رفع failureهای معتبر پیش از ZIP Production |
...
| اولویت | شناسه | وضعیت جاری | اقدام بعدی |
|---|---|---|---|
| P0 | `BUG-PROC-LINK-287 / FIN-WF-019` | resolver و لایهٔ انتخاب خرید واقعی با provenance پایدار در `v31.6.13` تکمیل شد؛ دادهٔ legacy بدون provenance عمداً انتخاب نمی‌شود | UAT و بررسی موارد legacy مبهم، بدون migration خودکار |
...
| اولویت | شناسه | وضعیت جاری | اقدام بعدی |
|---|---|---|---|
| P1 | `US-436` | دامنهٔ مصوب فعلی فقط «مازاد پروژه/انبار سبک» است؛ prototype موجود ولی load، route، sync، backup و اتصال salesfile ناقص | تصویب schema/lifecycle و Sprint مستقل؛ انبار کامل خارج از scope فعلی |
...
| Future | `WAREHOUSE-FULL-ROADMAP` | انبار کامل فعلاً نیاز نیست و در دامنهٔ US-436 نیست | فقط در roadmap توسعهٔ آینده بررسی شود؛ شامل دفترکل ورود/خروج، انبارها، انتقال و شمارش دوره‌ای |

**محدودیت:** US-436 در Sprint فعلی کدنویسی نمی‌شود؛ دامنهٔ فعلی فقط مازاد پروژه/انبار سبک است. انبار کامل در roadmap آینده ثبت شده و نباید به scope فعلی اضافه شود.

## Family 5 — فاکتور سپیدار/مودیان

| اولویت | شناسه | وضعیت جاری | اقدام بعدی |
|---|---|---|---|
| P1 | `US-437` | در `v31.6.13` rollback شده؛ فقط مستندات/کیس‌استادی | parked تا approval مستقل و طراحی workflow |
...
## گیت انتقال به baseline جدید v31.7

`v31.7` تا تکمیل hardening baseline فعلی آغاز نمی‌شود. معیار انتقال:

1. همهٔ P0های hardening و integrity دارای implementation، evidence و regression سبز باشند؛
2. regression کامل: `checks_fail=0` و `files_fail=0`؛
3. audit بدون error؛
4. release package flat-root و بدون داده/secret باشد؛
5. FIN-WF-001 Stage 0 حکم مستند داشته باشد؛
6. policy چک شخصی و staging وضعیت مشخص داشته باشند؛
7. handover، release note، baseline manifest و rollback plan برای baseline جدید آماده باشند.

تا آن زمان، اصلاحات hardening روی خانوادهٔ `v31.6.x` ادامه می‌یابد و US-436/US-437 و baseline جدید v31.7 آغاز نمی‌شوند.
