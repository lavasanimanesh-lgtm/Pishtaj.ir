...
## 11. وضعیت اجرا - گام 4 (تست‌های ادغام - تحویل شده - 1405/05/26)

### روش

تست در یک sandbox VM در محیط Node.js اجرا شد. توابع واقعی کد (نه mock) بار‌گذاری و روی داده‌های مصنوعی آزمایش شدند. نتایج: **43/43 قبول** - تمام الزام‌های 1،2،3،4،5 تأیید شدند.

### جزئیات سناریوها

| # | سناریو | نتیجه |
|---|---|---|
| T1 | صدور تک‌پیشنهاد با ویرایش قیمت | 11/11 PASS |
| T2 | صدور تجمیعی (2 پیشنهاد) | 5/5 PASS |
| T3 | collector پیشنهادهای متصل | 3/3 PASS |
| T4 | ابطال ریشه‌کن + cascade | 11/11 PASS |
| T5 | حفاظت وصولی‌ها (الزام 5) | 2/2 PASS |
| T6 | snapshot preserves | 4/4 PASS |
| T7 | guard های ابطال | 3/3 PASS |
| T8 | guard نقش | 1/1 PASS |
| T9 | اتصال به دفتر مشتری | 3/3 PASS |

فایل تست: `/tmp/step4_test.js` (محیط Node با VM sandbox)
گزارش: `STEP4-TEST-RESULTS-2026-08-17.md`
کامیت: `9c861fa`

### محدودیت تست

1. تست روی مرورگر واقعی اجرا نشده - sandbox DOM mock است.
2. **بدون تست سرور:** هماهنگی سرور (Sales Domain V2) فقط local.
3. توصیه می‌شود قبل از deploy نهایی، یک تست دستی روی CRM واقعی با پروندهٔ واقعی انجام شود.
