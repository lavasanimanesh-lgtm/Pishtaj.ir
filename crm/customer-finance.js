...
(function () {
  ...
  window.cfFinanceRowsHtml = function () {
    ...
    var table = rows.map(function (r) {
      ...
      return '<tr' + (open ? '' : ' style="color:#64748b"') + '><td><b>' + escP(r.co || r.cd) + '</b>' + (function(){ var en=(typeof ptfCustEnByCd==='function'&&r.cd)?ptfCustEnByCd(r.cd):''; return (en&&en!==r.co)?'<div style="font-size:10.5px;color:#64748b" dir="ltr">'+escP(en)+'</div>':''; })() + '<br><small style="color:#94a3b8;direction:ltr">' + escP(r.cd || '') + '</small></td><td style="font-weight:' + (open ? '900' : '400') + ';color:' + (open ? '#b45309' : '#64748b') + '">' + m(r.balance) + ' ریال</td><td style="color:#047857">' + (r.credit ? m(r.credit) + ' ریال' : '—') + '</td><td><button class="ba" onclick="cfOpen(\'' + ptfOnClickArg(r.cd) + '\')">📘 حساب</button></td></tr>';
    }).join('');
