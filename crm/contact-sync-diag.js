/* =====================================================================
   PTF CRM — contact-sync-diag.js — v34.39.33 (CONTACT-SYNC-DIAG-2 — SERVER-VERIFIED WRITE)
   ...
   قرارداد تازهٔ v34.39.33 — «ثبت روی سرور = فرمان + بازخوانی تأیید»:
...
(function () {
  ...
  /* krevs از لایهٔ سینک (اگر در دسترس باشد — A10)، وگرنه localStorage با try/catch */
  function readKrevs() {
    try {
    ...
    try {
      var p = JSON.parse(localStorage.getItem('ptf_sync_krevs') || '{}');
  ...
  */
  ...
  function onFixClick() {
    ...
    commitFix(plan.rec, lastServerRec, phone, function (res2) {
      ...
      if (res2 && res2.state === 'verified') {
        ...
        st.innerHTML = '<span style="color:#b91c1c">⛔ مسیر فرمان سروری (sales-domain-v2.js / entity_upsert) در این صفحه در دسترس نیست — <b>هیچ چیزی ثبت نشد و عمداً هیچ نوشتنِ محلی انجام نشد</b>. صفحه را با Ctrl+F5 کامل تازه کنید و دوباره امتحان کنید؛ اگر تکرار شد، نسخهٔ مستعر قدیمی/ناهمگن است — کل مجموعهٔ v34.39.33 را روی هاست بارگذاری کنید. (راه جایگزین: فرم ویرایش خودِ مشتری.)</span>';
  ...
  function runDiag() {
    ...
    fetchServerRec().then(function (sr) {
      ...
      if (e && e.keyMissing) {
        if (st) st.innerHTML = '⛔ سرور پاسخ داد اما کلید «مشتریان» را در پاسخ نیاورد (rev: ' + (e.rev || '—') + ') — این وضعیت نقش/نشست یا ناهمگنی نسخهٔ فایل‌های سرور را نشان می‌دهد. یک‌بار خارج و دوباره وارد شوید؛ اگر تکرار شد کل مجموعهٔ v34.39.33 را روی هاست بارگذاری کنید.';
      } else {
