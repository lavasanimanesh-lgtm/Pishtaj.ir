...
(function () {
  ...
  window.slOpenLedger = function (supCd) {
    ...
    var invRows = invs.map(function (i) { return '<tr><td>' + escP(i.dateFa || i.dateISO) + '</td><td><b>' + escP(i.no) + '</b></td><td>' + money(i.amount) + ' ' + escP(i.cur) + '</td><td>' + money(invRemain(i, d)) + ' ' + escP(i.cur) + '</td><td>' + (i.files || []).map(function (f) { return f.key ? '<a href="javascript:void(0)" onclick="openStoredFile(\'' + ptfOnClickArg(f.key) + '\')">📎</a>' : '📎'; }).join(' ') + '</td><td><button class="ba" onclick="slInvoiceEdit(\'' + ptfOnClickArg(i.cd) + '\')">✏️</button> <button class="ba" onclick="slInvoiceAddFile(\'' + ptfOnClickArg(i.cd) + '\')">📎</button> <button class="ba" style="color:#dc2626" onclick="slInvoiceVoid(\'' + ptfOnClickArg(i.cd) + '\')">ابطال</button></td></tr>'; }).join('');
