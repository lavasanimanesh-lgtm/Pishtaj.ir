...
(function () {
  ...
  function liquidityHtml() {
    try {
      ...
      /* v34.7.97 (FINHUB-UX-A): کارت‌های نقدینگی با فرمت فشرده — عدد کامل روی title.
         minmax از 150 به 170 افزایش تا در دسکتاپ کمتر ستون شلوغ. */
      var slMc = (typeof window.ptfMoneyCompactHtml === 'function') ? window.ptfMoneyCompactHtml : money;
      return '<div id="slLiquidity" style="background:#fff7ed;border:1px solid #fdba74;border-radius:14px;padding:12px 14px;margin-bottom:12px"><div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap"><div><b style="color:#9a3412">📊 تعهدات نقدینگی تأمین و چک‌های شرکت</b><br><small style="color:#92400e">چک ثالث منتقل‌شده در این شاخص وارد نمی‌شود.</small></div><button class="bt bt-o" style="font-size:11px" onclick="goPanel(\'sup\')">جزئیات تأمین‌کنندگان</button></div><div class="sr" style="grid-template-columns:repeat(auto-fit,minmax(170px,1fr));margin:10px 0 0"><div class="sc"><b>' + slMc(debt) + '</b><span>بدهی باز تأمین‌کنندگان</span></div><div class="sc"><b>' + slMc(credit) + '</b><span>اعتبار نزد تأمین‌کنندگان</span></div><div class="sc"><b>' + slMc(checks) + '</b><span>چک‌های شرکت در راه</span></div><div class="sc"><b>' + slMc(in7) + '</b><span>چک سررسید ۷ روز آینده</span></div><div class="sc"><b>' + slMc(in30) + '</b><span>چک سررسید ۳۰ روز آینده</span></div></div>' + (fxt ? '<small style="display:block;margin-top:8px;color:#92400e">مانده ارزی تأمین‌کنندگان: ' + fxt + '</small>' : '') + '</div>';
    } catch (e) { return ''; }
  }
