...
(function () {
  ...
  window.ptfEntitySaveCollection = function (collection, nextArr, opts) {
    ...
    /* v34.39.20 (RESTORE-TOMBSTONE-REKILL — گام ③): آرشیو حذف با هویت سرور (_id — idField
    ...
    /* v34.39.20: فرمان upsert آرشیو حذف به _id نیاز دارد (idField سرور) — ردیفِ هویت‌دارِ
  ...
  function caseBelongsToOffer(c,o){
    ...
    /* v34.39.20 (RESTORE-TOMBSTONE-REKILL — گام ⑤): رکوردِ «به جریان افتاده» offerNos را
  ...
  /* v34.38.17 (ORPHAN-ARCHIVED — گزارش کارفرما): پس از مختومه/بایگانی پروندهٔ فروش،
     ...
     origin='salesfile') و پیشنهاد برنده همان st='won' سالم می‌ماند. جاروی یکپارچگی
  ...
  /* v34.39.20: خروجی سراسری برای ptfSalesfileRestoreFromArchive — offerNos رکورد بازگشت */
  ...
  window.ptfOpenArchivedWonFile = function (projectNo) {
    ...
    var found = null;
    try {
      found = data('ptf_crm_projects').filter(function (p) { return p && (String(p.no || '') === projectNo || String(p.cd || '') === projectNo); })[0] || null;
    } catch (eArOpen) {}
    var no = found ? String(found.no || found.cd || projectNo) : projectNo;
    if (typeof window.openProject === 'function') { try { window.openProject(no); return; } catch (eOp) {} }
    if (typeof openProject === 'function') { try { openProject(no); return; } catch (eOp2) {} }
    alert('برای بازکردن پروندهٔ بایگانی، ابتدا بخش «پروژه‌ها/بایگانی» را از منو باز کنید (رکورد: ' + projectNo + ').');
  };
