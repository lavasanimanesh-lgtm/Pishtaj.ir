...
window.ptfArchivePurgeOpen=function(no){
  ...
  ptfArchivePurgeApi('archived_case_purge_plan',{projectNo:no}).then(function(d){
    ...
    var shellOnly=p.mode==='archive_shell_only',activeIds=(p.sharedActiveCases||[]).map(function(x){return x.cd||x.id;}).filter(Boolean);
    var modeNotice=shellOnly?'<div style="background:#fffbeb;border:2px solid #f59e0b;border-radius:12px;padding:11px;margin-top:10px;color:#92400e"><b>🛡 پرونده فعال مشترک شناسایی شد — حذف محدود به نسخه بایگانی است</b><br>پرونده فعال، پیشنهاد، RFQ، اسناد مشترک و همه آثار مالی آن حذف نمی‌شوند. فقط کارت/snapshot بایگانی و فایل‌های ابری منحصربه‌فرد آن پاک می‌شود.<br>پرونده فعال محفوظ: <span dir="ltr">'+escP(activeIds.join('، ')||'—')+'</span></div>':'<div style="background:#fee2e2;border:2px solid #ef4444;border-radius:12px;padding:11px;margin-top:10px;color:#991b1b"><b>⚠️ عملیات غیرقابل‌بازگشت مخصوص داده آزمایشی/اشتباهی</b><br>تمام رکوردهای عملیاتی و مالی متصل و '+(+p.cloudCount||0)+' فایل ابری حذف می‌شوند. مشتریان، کالاها و تامین‌کنندگان مشترک حذف نمی‌شوند.</div>';
    var body=modeNotice+'<div style="font-size:11.5px;color:#64748b;margin-top:6px">فقط tombstone حداقلی شناسه نسخه حذف‌شده برای جلوگیری از بازگشت داده از دستگاه قدیمی باقی می‌ماند و در گزارش سال مالی وارد نمی‌شود.</div><h4>گام ۱ — دامنه حذف را بررسی کنید</h4><div class="tb2"><table><thead><tr><th>مجموعه</th><th>تعداد</th><th>نمونه شناسه</th></tr></thead><tbody>'+rows+'</tbody></table></div><div style="margin-top:8px;font-size:12px;color:#475569">جمع رکورد: <b>'+(+p.recordCount||0).toLocaleString('fa-IR')+'</b> | فایل ابری: <b>'+(+p.cloudCount||0).toLocaleString('fa-IR')+'</b> | سال‌های مالی متاثر: <b>'+escP((p.years||[]).join('، ')||'—')+'</b></div><h4>گام ۲ — تایید ماهیت آزمایشی</h4><label style="display:block;padding:7px"><input type="checkbox" id="arcPurgeTest"> '+(shellOnly?'تایید می‌کنم فقط نسخه بایگانی یک کپی آزمایشی/تکراری است و پرونده فعال باید حفظ شود.':'تایید می‌کنم این پرونده و داده‌های متصل به آن آزمایشی/اشتباهی هستند.')+'</label><label style="display:block;padding:7px"><input type="checkbox" id="arcPurgeNoRecovery"> می‌دانم اطلاعات و فایل‌ها پس از حذف قابل بازیابی از CRM نیستند.</label><div class="fld"><label>دلیل حذف قطعی *</label><textarea id="arcPurgeReason" rows="2" placeholder="مثلاً: پرونده آزمایشی قبل از بهره‌برداری واقعی"></textarea></div><div class="fld"><label>برای تایید، شماره پرونده را دقیق وارد کنید: <b dir="ltr">'+escP(no)+'</b></label><input id="arcPurgeTyped" autocomplete="off" dir="ltr"></div><div style="display:flex;justify-content:flex-end;gap:8px"><button class="bt bt-o" onclick="ptfArchivePurgeClose()">انصراف بدون تغییر</button><button class="bt" id="arcPurgeCommit" style="background:#b91c1c" onclick="ptfArchivePurgeCommit(\''+ptfOnClickArg(no)+'\',\''+ptfOnClickArg(p.planHash)+'\',\''+ptfOnClickArg(p.keysDigest)+'\')"> '+(shellOnly?'حذف فقط نسخه بایگانی و حفظ پرونده فعال':'حذف قطعی تمام داده‌های این پرونده')+'</button></div>';
...
window.ptfArchivePurgeCommit=function(no,planHash,keysDigest){
  ...
  var preCommitFailed=function(e){alert('⛔ پیش‌نیاز حذف قطعی متوقف شد: '+e.message);if(btn){btn.disabled=false;btn.textContent=(window._ptfArchivePurgePlan||{}).mode==='archive_shell_only'?'حذف فقط نسخه بایگانی و حفظ پرونده فعال':'حذف قطعی تمام داده‌های این پرونده';}};
  ...
  ptfArchivePurgeApi('archived_case_purge_plan',{projectNo:no}).then(function(d){
    ...
    if(typeof window.ptfSalesDomainCommand!=='function')throw new Error('موتور قطعیت فرمان سرور بارگذاری نشده است');
    window.ptfSalesDomainCommand('archived_case_purge_commit',{projectNo:no,typedProjectNo:typed,reason:reason,planHash:planHash,testDataConfirmed:true,confirm:'PTF-PURGE-ARCHIVED-TEST-CASE',idempotencyKey:'PURGE-ARCHIVE|'+no+'|'+planHash.slice(0,16)}, {
      onAck:function(d){var r=d.result||{};ptfArchivePurgeClose();try{hideModal();}catch(e){}if(typeof renderProjects2==='function')renderProjects2();if(typeof ptfFiscalRender==='function')ptfFiscalRender();if(typeof ptfToast==='function')ptfToast(r.mode==='archive_shell_only'?'نسخه بایگانی حذف شد و '+(+r.sharedActiveCases||0)+' پرونده فعال مشترک محفوظ ماند':'پرونده آزمایشی و '+(+r.recordCount||0)+' رکورد مرتبط به‌طور قطعی حذف شد','warn');},
      onReject:function(e){alert('⛔ ثبت حذف رکوردها رد شد: '+e.message+'\nفایل‌های ابری قبلاً طبق receipt حذف شده‌اند؛ کیفیت داده باید بررسی شود.');if(btn){btn.disabled=false;btn.textContent='بازبینی نتیجه حذف';}},
      onUncertain:function(e){alert('⚠️ نتیجه ثبت حذف رکوردها هنوز نامشخص است. همان operationId حفظ شده؛ دکمه را برای بازیابی همین فرمان بزنید. شناسه: '+e.operationId);if(btn){btn.disabled=false;btn.textContent='بررسی نتیجه / تلاش مجدد';}}
    });
  },preCommitFailed);
};
